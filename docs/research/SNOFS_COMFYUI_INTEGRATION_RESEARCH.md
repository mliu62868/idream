...
仓库曾有 `darkbeast-flux2-klein-9b-multi-reference.json`（历史候选，当前仓库已移除），其 `VAEEncode → ReferenceLatent`、语义 image slot 和 `ComfyUIBackend` 绑定结构可作为设计参考。SNOFS 无需改 backend；应新增独立 descriptor，而不是改现有 Qwen 或 Dark Beast：
