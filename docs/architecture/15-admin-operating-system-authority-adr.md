...
5. 跨服务只在 durable receipt/canonical row 已提交后 ACK。Generation provider 调用前先登记 TransportExecution；自动 transport retry 只允许 deterministic provider idempotency。provider 结果先写 immutable completion manifest，ACK 中断只重投 manifest，不再次调用 provider。
