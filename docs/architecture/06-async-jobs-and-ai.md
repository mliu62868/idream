...
本文件落地 ADR-5（无 Redis 队列）与 ADR-6（AI provider 抽象），以及生成/聊天的异步流水线与 dreamcoin 预留结算。队列清单对齐 `BackendFeatureSpec §7`。

## 1. 为什么异步

同步 HTTP 路径**禁止**调 AI / 做重 IO（01 §8）。聊天生成、图/视频生成、审核、webhook、奖励发放、摘要——全部入队，由 worker 处理。好处：serverless 不超时、可重试、可观测、可限并发、可控成本。
...
queued ──claim──▶ running ──ok──▶ completed
   ▲                  │
   │ (退避重排)         └─fail&可重试─▶ queued(nextRunAt=now+backoff, attempts++)
   └──────────────────┘
                      └─超过 maxAttempts──▶ dead（死信，留证 + 告警）
```
...
export interface JobQueue {
  ...
  claim(worker: string, max: number): Promise<Job[]>;      // 原子认领
  complete(id: string): Promise<void>;
  fail(id: string, err: string, retry: boolean): Promise<void>;
}
```
...
worker 处理循环：`claim → 按 queue 分发到 handler → complete/fail`，单次 drain 处理一批后返回（受 `maxDuration` 约束），剩余下次 Cron 继续。
...
| `chat.generate` | chat API | 拼 prompt → ChatModel 流 → 输出审核 → 落 assistant message/version → 更新 usage/memory | messageId |
| `generation.image` | generation API | 输入审核 → ImageModel → 输出审核 → 落 MediaAsset → 结算 coins | jobId |
| `generation.video` | generation API（P1） | 同上，VideoModel | jobId |
...
| `chat.summarize` | chat worker | 压缩旧消息进 memorySummary | sessionId+watermark |
...
见 05 §4 与 04 §8。要点：
- 同步路径：审输入 → 落 user message → 入队。
- worker/SSE：拼 prompt（system + memory + 窗口）→ `ChatModel.stream` → 边流边发 SSE → 全文输出审核 → 落 assistant message + version → usage/memory 更新。
- 额度：免费用户按 `chat_usage` 限；`unlimited_messages` entitlement 跳过；模型能力按 plan（Deluxe = premium models + 3x memory）。
