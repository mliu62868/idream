...
图片生成拆成三个子门槛，避免把内部 Alpha、配置中心和真实 Pipeline Beta 混在一个交付里。

### M6a · 图片生成 Alpha
**交付**：`generation/jobs`（dreamcoin reserve 06 §6）、`generation.image` worker(mock 或 sandbox pipeline)、输入/输出审核接口、`BlobStore`(私有签名)、`MediaAsset`、`GET /media`、like/download/delete、admin job detail。
**退出**：选角色/Freeplay+preset→生成→完成媒体进 Images；失败退款；部分成功展示成功张数和退款；签名 URL 访问。E2E 流 5。

### M6b · 生成配置中心
**交付**：`GenerationModelProfile`、`GenerationPromptTemplate`、built-in preset 管理、`generation/config` 从 active profile/template 读取、publish/rollback/audit、dry-run sample matrix。
**退出**：Admin 可以发布/禁用/回滚 model profile 和 prompt template；旧 job 保留 profile/template version；非 admin 不能改配置。

### M6c · 真实 Pipeline Beta
**交付**：`packages/gen` 接 `PipelineImageModel`，内部 Pipeline Service 首选 `stable-diffusion.cpp` runner，真实私有 BlobStore，provider error mapping，profile health metrics。
**退出**：真实图片从生成到图库跑通；按 profile 看到成功率、延迟、退款率、blocked 率；可暂停异常 profile。
...
## M9 · 审核后台（spec P0 #10）
**交付**：`admin/moderation/queue`（按优先级，未成年=1 即时隐藏）、`admin/.../decision`（写 review + 改目标态 + 审计）、admin 鉴权、generation job 排障、用户/ledger 查询、死信重放、audit log。
**退出**：举报/角色/消息/媒体能进队列并被处置；生成任务能排查 provider/profile/ledger/media timeline；决定有 policyCode + 审计。E2E 流 7。
...
| 门槛 | 定义 |
| --- | --- |
| Internal Alpha | mock/sandbox provider 可用，业务状态、ledger、media、admin job detail 闭环 |
| Closed Beta | 真实 Pipeline API、私有 BlobStore、基础 output moderation、Premium gate、失败/退款 UI 和配置回滚 |
| Public Launch | 真实 CSAM/未成年检测与上报 runbook、辖区年龄验证策略、真实支付、举报/申诉、审计和监控全部就位 |
...
## ⚠️ 暂缓项（Deferred TODO，公开上线前必须补齐）

按当前决策，以下外部供应商/合规/运营项 **当前阶段用 mock / 占位 / 本地实现推进**，但**在面向真实用户、承载真实内容上线前必须补齐**（合规、资金、数据与运营安全硬门）：

| TODO | MVP 现状 | 上线前必须 | 关联 |
| --- | --- | --- | --- |
| **Safety Gateway** | 保留 moderation 接口/事件、举报队列、角色 age≥18 硬规则、关键词/mock provider | 接真实 text/media Safety Gateway，并配置 `MODERATION_PROVIDER=safety-gateway`、`CHAT_MODERATION_PROVIDER=safety-gateway` | 07 §3.2 / M5 / M9 |
| **Go.cam** | 保留 `age_verifications` 表与守卫，默认 `not_required` 直通 | 接 Go.cam 或等价 provider，按辖区触发，配置公网 HTTPS callback/link-back | ADR-7 / 07 §2.3 / M1 |
| **BTCPay** | mock 支付 provider 跑通计费链路 | 接 BTCPay Greenfield，testnet→mainnet，配置 webhook 验签与幂等结算 | ADR-4 / 08 / M7 |
| **R2/S3** | 本地 blob / mock storage 验证媒体链路 | 接私有对象存储 bucket、签名 URL、write/read/delete probe | M6 / launch storage probe |
| **Sentry** | 本地日志和测试覆盖 | 配置 `SENTRY_DSN`，接入生产错误监控与告警责任人 | launch observability probe |
