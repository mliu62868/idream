...
| --- | --- | --- | --- |
| premium | monthly prepaid term | $19.99 / 30-day term | 1,500 dreamcoins、unlimited messages、imageGeneration、voiceEnabled、voiceMinutes=30、videoGeneration=false |
| premium | yearly prepaid term | $99.90 / annual term | 18,000 dreamcoins、unlimited messages、imageGeneration、voiceEnabled、voiceMinutes=360、videoGeneration=false |
| deluxe | monthly prepaid term | $59.99 / 30-day term | 6,000 dreamcoins、Premium 全部 + premiumModels、voiceMinutes=120、videoGeneration=true（仅在 `video_gen` + provider ready 时曝光） |
| deluxe | yearly prepaid term | $299.90 / annual term | 72,000 dreamcoins、Premium 全部 + premiumModels、voiceMinutes=1440、videoGeneration=true（仅在 `video_gen` + provider ready 时曝光） |
...
## 2. 预付访问生命周期（物理模型仍沿用 `Subscription` 名称）

加密支付**无卡式自动续费**（钱包不能被拉扣），故付费访问按"一次性预付周期 + 到期后用户重新购买"建模（ADR-4）：

```
invoice_created → awaiting_payment → confirming(链上确认中) → active(至 periodEnd)
                                   → underpaid / expired_window → canceled
   重新购买：periodEnd 前后再次付款 → 按购买时 offer 激活新的权益周期
   未重新购买：到 periodEnd → expired（只移除未来高阶权益）
```

映射 spec §4.5：保留 `active|canceled|expired`；`past_due` 仅是 legacy enum 兼容值，当前 provider 不用它表达续订或催款。尚未付款/确认的状态属于 checkout/invoice，不属于已激活访问周期。
...
- **激活**：**只由 IPN/webhook + 足够链上确认驱动**（§3），不信前端回跳；确认到账 → `currentPeriodEnd = now + period` + 发权益 + 本次 offer 的 included dreamcoins。
- **到期/重新购买提醒**：临期由 cron 提醒用户可再次购买；不创建自动扣款或续订承诺。
...
worker billing.webhook（必须等"已确认/已结算"才发权益）:
  - confirmed/finished/settled → 付费访问记录 active + currentPeriodEnd=now+period；重算 entitlements（§4）；入队 reward.ledger 发本次 offer included dreamcoins / 充值 coins
...
**不变量**：
- 一切付费访问记录状态变更**来自验签通过 + 足够链上确认的 IPN**，不信客户端回跳（spec §4.5）。
...
**奖励来源**：`signup_bonus`、`subscription_grant`（legacy reason；每次购买激活发放）、`redeem`、`referral`、`admin_adjust`，全部经 `reward.ledger` 队列恰好一次。
...
- **dreamcoin 是平台唯一消耗型货币**（决策见 ECONOMY §0）。当前计划卡展示 `includedDreamcoins`、消息权益与生成模型权益，不展示 images/videos/voice quota；Chat provider/model、角色人格和基础记忆不按计划分级。若未来恢复媒体等价数字，必须按 `includedDreamcoins ÷ 费率` 动态计算，且 `video_gen=false` 时不得展示视频承诺。
...
正常已结算 prepaid access record（物理模型名仍为 `Subscription`）的全额退款不走 `admin_adjust`：
...
- 加密特性：等确认才发权益、无自动续费（到期提醒后由用户重新购买）、处理欠付/超付/发票过期（§2/§3）。

## 9. 验收（对齐 spec §9）

- [ ] Premium/Deluxe 门服务端 entitlement 强制。
- [ ] dreamcoin 全为 append-only ledger 条目，余额可由 ledger 重算。
- [ ] 付费访问记录状态仅由验签 + 幂等 webhook 改变；当前 provider 不产生自动续订。
