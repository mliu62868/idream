...
> **Admin Release 实现补充（2026-08-28）**：`active CharacterVisualProfile`、sealed `ReferenceSetRevision` 与 pinned compatible generation route 是三个独立事实；三者与精确 generation provenance 一起被 immutable Character Release snapshot 固定。运营点击一次 Publish，服务端自动执行技术检查并切换 Serving；不再设置角色级 QA、人工发布审核或定时发布。角色运营每个 Run 强制 `count=1`，逐张选择素材；旧的 40 张模型评测矩阵只保留为可选技术评测证据。profile/workflow 漂移会使已选草稿 lineage stale，但不会静默改写历史证据或自动下线当前 Serving。
...
iDream 的完整 Generate 同时包含 Character 和 Freeplay 任务。本文只定义 Character 模式：它不是“输入 prompt，返回一张与角色无关的图片”，核心承诺是：

> 用户每次看到的都是同一个角色，只是她正在经历不同的时刻。

因此 Character 模式的交付单位是可被 Chat、Gallery、角色资产和后续创作引用的“角色媒体”，而不是一张孤立图片。Freeplay 仍是独立、完整的 Generate 任务，不强制绑定角色或关系上下文。Character 模式需要同时保证：
...
角色图片在 Chat、Create、Generate 和 Gallery 中承担五个任务：

| 任务 | 用户感受 | 产品结果 |
|---|---|---|
| 身份确认 | “她还是她” | 角色与视觉可信度 |
| 场景具象化 | “我看见了我们刚才聊的那一刻” | 对话沉浸与继续聊天 |
| 上下文资产 | “这是这个角色在当前场景中的图” | 收藏、回看、引用和后续创作 |
| 个性表达 | “这是我塑造出来的角色” | 所有感与创作者投入 |
| 付费价值 | “更稳定、更快、更可控” | 生成消费和订阅转化 |

在 Character 模式中，漂亮但不像目标角色的图仍是身份交付失败；Freeplay 则按独立的 prompt/intent 与 artifact 质量契约验收。
...
子系统不以“生成次数”作为价值证明，主结果指标是：

> 每 100 个 Character-mode 生成意图中，成功交付且被查看、收藏、设为角色资产、引用或继续创作的有效角色媒体数。
...
| Media Continuation Rate | 图片完成后 10 分钟内继续 Chat、编辑或基于图片继续创作 |
| Visual Asset Reuse Rate | 图片被收藏、设为主图、存 Look、加入 reference 或后续引用 |
