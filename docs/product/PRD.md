...
本文档基于对标站 Ourdream.ai 的产品调研、已保存截图、当前源码和现存研究资料（`docs/research/INSPECTION_GUIDE.md`、`docs/research/SERVICE_INTEGRATION.md`），整理 Ourdream.ai 目标产品的完整产品功能、页面信息架构和实现范围。
...
| CH-03 | 聊天应支持历史会话、上下文记忆和角色设定注入。 | P0 |
| CH-04 | 用户可重新生成回复、编辑上一条、删除会话。 | P1 |
| CH-05 | 支持聊天内生成图片或跳转生成器。 | P1 |
| CH-06 | 免费用户和 Premium 用户的消息额度、速度或模型能力需要区分。 | P1 |
| CH-07 | 聊天必须接入安全策略、举报和内容限制。 | P0 |

### 6.4 角色创建器

| ID | 需求 | 优先级 |
| --- | --- | --- |
| CR-01 | 用户可选择角色 Gender：Female、Male、Trans。 | P0 |
| CR-02 | 用户可选择 Style：Realistic、Anime。 | P0 |
| CR-03 | 用户可选择外观/race、发型、体型等多步属性，并支持 Custom。 | P0 |
| CR-04 | 用户可编辑或接受生成的角色名称。 | P0 |
| CR-05 | 用户可打开 Advanced Details 补充角色设定。 | P1 |
| CR-06 | 用户可管理创建前 tags。 | P0 |
| CR-07 | 系统可在创建前生成/刷新预览图。 | P1 |
...
> **单一货币模型**：dreamcoin 是平台**唯一**消耗型货币，图片/视频按费率扣币，语音优先消耗计划分钟额度、超出后按 clip 兜底扣币；**不存在独立的图片/视频配额计数器**。当前 beta 计划卡展示 dreamcoins 与聊天/模型权益，不展示「images/videos/voice」等价示意；若未来恢复媒体等价展示，必须由代码动态算出，且 `video_gen=false` 时不得展示视频承诺。价格、费率、免费档、退款的单一事实来源（SSoT）为 `ECONOMY_AND_PRICING.md`，本节不重复费率卡。
...
| ID | 需求 | 优先级 |
| --- | --- | --- |
| SE-01 | 每个 sitemap 路由应有可索引页面、title、description 和 canonical。 | P0 |
...
| Subscription | id、userId、plan、status、billingPeriod、provider、providerSubscriptionId、currentPeriodStart、currentPeriodEnd（公开映射为 benefitsEndAt，不代表 renewsAt） |
