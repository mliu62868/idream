...
| 范围 | 证据 |
| --- | --- |
| 全量 E2E | `PW_WEBSERVER=1 PW_BASE_URL=http://127.0.0.1:3064 PW_ADMIN_BASE_URL=http://127.0.0.1:3001 bun run --filter @idream/main test:e2e`，76/76 passed |
...
| Public catalog | 2026-07-03 `.tmp/public-catalog-probe-2026-07-03-continuation.json`，16 public characters、13 creators、16 distinct images、0 issues |
...
| launch gate | 2026-07-04 `.tmp/check-launch-current-root-2026-07-04-goal.json`：`bun run check:launch -- --json` 为 `7 pass / 49 fail / 2 warn`，且根命令现在写出可解析 JSON；`.tmp/check-launch-production-example-2026-07-04-goal.json`：`bun run check:launch:direct -- --launch-env-file packages/main/.env.production.example --json` 为 `31 pass / 33 fail / 0 warn`；`.tmp/check-launch-production-example-fresh-mock-2026-07-04-goal.json` 证明新鲜 mock payment/blob/age 报告仍被 production-shaped gate 拒绝；`.tmp/check-launch-production-example-2026-07-04-video-template.json` 证明 production example 下 video provider check 以“video disabled”通过。三者都仍未达到公开上线 gate |
