...
| Load/Chaos | 部分（隔离 transport rehearsal 已验证） | `admin-production-like-readiness.ts` 在 rollback-only 的生产表/索引上覆盖 100k Jobs/Cases、1m Events 与真实 Today handler；Jobs 通过 Route Handler 同源 `queryGenerationJobsV2Authority` 执行服务端 search/filter/composite cursor 和共享 DTO 投影，校验精确 total、`page <= limit` 与 p95 `<1000ms`；Events 执行真实且结果有界的 count/name/authority-class 聚合，校验 1m 精确计数、完整 coverage 与 p95 `<1000ms`。2026-07-12 最终实跑 Today p95 `532.874ms`、Support `408.721ms`、Jobs `94.012ms`、Events `238.292ms`。`admin:readiness:chaos` 在专用端口由父 harness SIGKILL 临时 PostgreSQL/Redis 与挂起在精确故障点的 consumer/dispatcher/projector 子进程，再启动全新 recovery 子进程验证 lease recovery、commit-before-LREM ACK、无 ghost queued、provider boundary 单调用、Audit/Outbox/receipt 单副作用、watermark catch-up 与 replay no-op；scheduled publish 并发另由 `release-executor.integration.test.ts`、`release-monitor.integration.test.ts` 锁定 | 该 load 证据不等同于完整 metric projector 或网络分区演练；chaos harness 使用隔离的最小 `chaos_*` transport schema，也不等同于对真实 Prisma/领域模块做进程 kill。网络分区与真实模块 L3 仍须在集成环境演练。进入 schema-v5 Gate 前还必须保存为不可变 artifact 并由受信 collector 签名。 |
