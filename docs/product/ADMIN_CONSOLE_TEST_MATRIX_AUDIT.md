...
| Contract | 已验证（本地边界） | `packages/shared/src/admin/**/*test.ts`、`contract-runtime-parity.test.ts`、BFF runtime gate 与各 Route Handler integration；76 个真实 route files 精确枚举 84 个 HTTP operation；130/130 个唯一 contract ref 均为可执行 Zod/transport binding、pending=0，并自动跑正反 fixture。Admin BFF 对全部 84 个成功响应执行生产路径 schema 校验，非法响应 fail closed 为 502；真实浏览器链路据此发现并修复 Creative/Incident/Case mutation DTO 漂移 | 并非每个 handler 都在单一测试中用业务 fixture 主动生成成功响应；生产 canary 仍是外部 Gate。 |
...
| Load/Chaos | 部分（隔离 transport + 真实 Prisma projector 进程 seam 已验证） | `admin-production-like-readiness.ts` 在 rollback-only 的生产表/索引上覆盖 100k Jobs/Cases、1m Events 与真实 Today handler；Jobs 通过 Route Handler 同源 `queryGenerationJobsV2Authority` 执行服务端 search/filter/composite cursor 和共享 DTO 投影，校验精确 total、`page <= limit` 与 p95 `<1000ms`；Events 执行真实且结果有界的 count/name/authority-class 聚合，校验 1m 精确计数、完整 coverage 与 p95 `<1000ms`。2026-07-12 最终实跑 Today p95 `532.874ms`、Support `408.721ms`、Jobs `94.012ms`、Events `238.292ms`。`admin:readiness:chaos` 在专用端口由父 harness SIGKILL 临时 PostgreSQL/Redis 与挂起在精确故障点的 transport 子进程，再启动全新 recovery 子进程验证 lease recovery、commit-before-LREM ACK、无 ghost queued、provider boundary 单调用、Audit/Outbox/receipt 单副作用、watermark catch-up 与 replay no-op。`admin:readiness:chaos:real-projector` 另启动真实 `projectCanonicalMetricEvent` + Prisma 子进程，在 fact 已写而 receipt 未写的事务内故障点由父进程 SIGKILL，证明事务整体回滚；新进程随后 apply、再次新进程 replay，最终 canonical fact 与 `MetricProjectionReceipt` 均精确为 1。scheduled publish 并发另由 `release-executor.integration.test.ts`、`release-monitor.integration.test.ts` 锁定 | 隔离 transport schema 不替代真实 BullMQ consumer、Admin command worker/dispatcher 与网络分区演练；当前已关闭真实 canonical metric projector process-kill seam，剩余真实模块 L3 仍须逐项演练。进入 schema-v5 Gate 前还必须保存为不可变 artifact 并由受信 collector 签名。 |
...
| Redis 丢失、receipt 后 projector 崩溃、payload conflict、main→chat 丢失 | 已验证（隔离 transport + 真实 Prisma projector process seam） | `admin:readiness:chaos` 实际 SIGKILL/restart 专用 Redis，并由父 harness SIGKILL fault 子进程、启动全新 recovery 子进程；`admin:readiness:chaos:real-projector` 强杀事务内真实 Prisma projector 并由新进程恢复/重放；main/chat outbox、durable ingest、payload conflict、event consumer recovery tests 补齐其余领域 seam。 |
...
- production-like 网络分区，以及直接运行真实 BullMQ consumer、Admin command worker/dispatcher 等剩余领域模块的进程 kill L3 演练；真实 Prisma projector 已有本地 process-kill seam，但单项通过不替代其余证据。
