...
- `bun run launch:probe:generation-model-candidates -- --candidate redcraft_krea2_default,redcraft_krea2_text` 审计当前默认与隔离候选。`redcraft_krea2_default` 必须精确解析为 `runner=comfyui`、`pipelineModel=redcraft-krea2-comfyui`、`workflowKey=redcraft-krea2-txt2img`，并保持 `active + enabled=true + rolloutPercent=100 + readyForPublish=true`；用 `--candidate redcraft_krea2_default --require-ready` 验收。`redcraft_krea2_text` 仅是历史兼容候选，必须保持 `draft + enabled=false + rolloutPercent=0`，不得以 `--require-ready` 误当当前默认。旧 `pornmaster_zimage_default` 输入只保留为 probe alias，并在报告中规范化为 `redcraft_krea2_default`，不能再作为运行手册里的权威 key。`bun run launch:probe:redcraft-comfyui` 只验证 Redcraft 当前 ComfyUI workflow 能否出非退化图；`bun run launch:probe:redcraft-image:local` 验证它能通过统一 gen pipeline 写入 blob；`bun run launch:probe:redcraft-consistency:local` 生成 20 张样本和 review 页面，默认使用 `seedMode=locked` 以模拟主站 `CharacterVisualProfile.defaultSeed`。
