...
The 2026-07-11 Admin correctness review found that several Admin surfaces were interactively covered without certified state/metric semantics. The implemented foundation now includes shared v2 contracts/permissions, migration bootstrap, Generation/Creative truth containment, idempotent command/event reliability, durable chat/main/gen ACK seams, authoritative command acceptance, reconstructable Today queues, a fail-closed Admin HTTP BFF bootstrap, and an executable §19.4 cutover-invariant ledger that blocks on nonzero or unavailable checks. Character/Release/Visual now has resumable backfill/reconciliation, exact immutable evidence, schedule/publish/rollback executors, Serving/Audit/Outbox/monitor atomicity, a recoverable command worker, V1 state adapter delegation, immutable Chat Session Release pins, explicit verified next-turn migration, derived route-stale dispatch, and canonical 24h/72h monitor collection. Creative Retry/Generation now has attempt-level retry executors, durable dispatch, lease/result verification, immutable review decisions, distribution-owned placement publishing, current-slot verification, and v2 lineage/read APIs; release-owned placement remains under Character Release authority. Incident and P0 Review Case now have authoritative aggregation/backfill, assignment/SLA, Evidence/Decision, action-plan preview→execute, recovery verification/override, resolve/close executors, and V1 Report/Appeal adapter delegation. Metrics now adds typed chat exchange provenance, generation delivery/AiUsage events, canonical facts, a versioned Registry, golden cohort calculations, fail-closed quality gates, `/api/v2/admin/metrics`, resumable backfill/reconciliation tooling, and a real experiment authority with stable versioned assignment, idempotent typed exposure facts, and mature exposed-cohort directional QCE lift. NS-01 remains open: WPCU is official and WSCU/WSCrU/WPSCU remain shadow. Remaining work includes executing production authority backfills and explaining two complete mature shadow windows; replacing job-linked terminal evidence with attempt-linked canonical terminal facts; Character Project/preview/Portfolio, production qualification eval scheduling, production Session-pin reconciliation/canary, and mature monitor windows; complete responsive/a11y/browser UI coverage for the Creative review/placement workflow; deeper Incident/Case downstream workers and UI; contribution margin, experiment management UI and guardrail/statistical evaluation; remaining roots in Today; removing the Admin UI/CSS main-source alias; broad legacy cutover, canary, and observation gates. UI availability, additive schema, calculable local metrics, command acceptance, or one vertical slice alone must not be counted as final cutover. The remediation plan above remains the execution SSoT.
