...
The 2026-07-11 Admin correctness review found that several Admin surfaces are interactively covered but do not yet have certified state/metric semantics. The first implementation batch now covers metric truth containment, shared v2 contracts/permissions, Generation/Creative truth, migration bootstrap, additive authority models, command/event idempotency, durable receipt/outbox primitives plus a minimal production chat↔main durable ACK and gen completion-manifest vertical slice, Character Release readiness on every transition into the approved state, fail-closed lease recovery that only replays explicitly idempotent commands, v2 command status plus four authoritative POST acceptance endpoints, and a reconstructable Today projection over AdminCase/OpsIncident/ControlPlaneCommand with server-side permission scopes and exact totals. Remaining Admin work starts at backfill/shadow, domain executors and verification for the four workflows, bringing the remaining Character/Creative roots into Today, and broad migration of legacy direct queue producers; UI availability, additive schema, command acceptance, or the initial durable seam alone must not be counted as final cutover. The remediation plan above supersedes older completion assumptions and remains the execution SSoT.
