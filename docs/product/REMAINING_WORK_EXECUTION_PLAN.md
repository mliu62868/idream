...
The repository, automated journeys, and current service probes prove a working local MVP implementation. The code-owned core chains, ADR-19 single DSH/official-igrep runtime migration, and both production video recipes are closed in the controlled local environment. The final revision still has not received one complete authenticated Chrome rerun against a public production envelope, so the active milestone remains **internal demo / controlled beta**, not public launch.

The latest executor-bound structured report is `.tmp/check-launch-video-runtime-migration-2026-08-23-executor-final.json`: `37 pass / 31 fail / 0 warn` under `LAUNCH_SCOPE=core`; payment and age verification remain excluded. It binds the signed Chat E2E, exact LTX/H3 direct workflows and model bytes, and both Main persistence reports to executor revision `idream-worktree-0fdf96b06508a8838f5d4fadb9157ad293e22ffb8f6b3737fba35f1768862069`. The five migration-critical checks all pass. Main migration authority remains exact `71/71`, Chat provider/memory readiness is healthy, and the final ComfyUI queue is empty. Public launch remains blocked by 31 production-envelope checks: public HTTPS, production secrets/tokens, Redis/BullMQ, non-mock Blob, Sentry, fresh Recovery and other stale production probes. Because the worktree digest includes documentation, this documentation update makes the `0fdf…` report an immutable historical snapshot; a new release must restart and reprobe rather than relabel it.

Account Deletion's code-owned rollback boundary is closed with `user.account_deletion.requested.v2`: the event has a dedicated current-Chat ingest path, stays pending on mixed-version 404s instead of exhausting into `failed`, and is absent from the rollback Main sender whitelist. The local PostgreSQL 16 runtime and fresh/upgrade/redeploy/rollback rehearsal are now 71/71. In addition to the run-owned Playwright canary, an approved one-time customer was deleted through the user's Chrome on the current product runtime. The original 30-day grace fact was preserved; only that actor's AccountDeletion and request Outbox rows were atomically advanced under exact CAS, then the real Main→Chat HTTP path, Chat PG/FS erasure, request-bound completion, Blob receipts and Main hard-delete reached `completed`. Login was rejected immediately after the browser request, both generated Voice assets were removed at Blob finalization, and no unrelated user data was touched.

The complete numbered authenticated customer/Admin Chrome suite recorded in the full-product audit predates the final backend patch, so it is not represented as one current-revision suite. After the patch and Recovery restart, current real Chrome separately rechecked the public character detail at 1280 and emulated 390x844, visible Tab focus on Explore, the Chat CTA's expected 401→signup boundary, BFCache recovery, an entitled Mara Chat opening/reply Voice generation with natural-ended playback and reload persistence, and the approved disposable Account Deletion terminal chain described above. The signed Chat probe remains the repeatable authenticated runtime transport evidence; these Chrome journeys are narrower user-path evidence, not a production-capacity claim.

### 2026-08-12 active closure queue

1. **Completed locally 2026-08-12:** the four pending migrations were applied to the PostgreSQL 16 runtime after a checksummed three-layer snapshot; exact migration authority is 71/71, legacy Premium v1 is archived, RedMix3 v2 is active, Product Config is green, and Voice/Account Deletion postconditions pass. Production deployment must repeat this controlled sequence against its own pre-migration checkpoint rather than reuse local evidence.
2. **Completed 2026-08-11, revalidated 2026-08-12:** the read-only audit proved all 48 envelopes were payload-valid target-missing callbacks with no Chat receipt, receiver attachment, source outbox or replayable effect. Admin Chat Operations → `Record target missing` persisted 48 Chat `discarded_target_missing` receipts before Main CAS/audit terminalization and explicitly recorded no user effect. A later audit found that ordinary Chat Vitest could resolve the ambient Main ingest URL; the test boundary now permits cross-service writes only from an explicitly run-owned Playwright environment, and the exact leaked fixtures were removed. The current post-cleanup audit is `ok=true`, failed/replayable/reconciliation-required are all zero, and the launch backlog check passes.
3. **Completed locally:** the current authority is `.tmp/recovery-bundles/idream-recovery-local-20260814-final-user-journeys`. Its checksummed metadata and structured Gate output are the authority for completion time, master-manifest digest and source identity; tracked documentation intentionally does not duplicate those per-run values. PostgreSQL 16 migration 71/71 source/isolated restore, schema/logical/counts, Chat FS, local mock Blob, DB/queue authority and quiescence receipt are exact. The maintenance contract blocks in-flight work only; stable scheduled/pending/failed durable backlog is preserved unchanged and compared exactly. Production must repeat the contract against its own data and independent non-mock recovery authority; the local bundle is not production evidence.
4. **Completed locally 2026-08-14:** the canonical Chat boundary is applied and the final source-bound signed-BFF service probe is `ok=true`. Current verification is Shared 46/254, Gen 21/202, Main 315 passed files + 2 skipped / 2,479 passed tests + 3 skipped, Chat 37/348, and Admin 118/586; repository total is 537 passed files + 2 skipped / 3,869 passed tests + 3 skipped. Turbo test tasks are 6/6 with Chat forced by `cache:false`; typecheck is 6/6, lint 2/2, production build 5/5, and PM2/operator/source-revision tests 84/84. The final runtime observation has ten iDream PM2 app instances online and the same source revision in every definition; Chat `/readyz` is 200. Image 2/2 plus Video 1/1 ownership and Generation cutover report `issues=[]`; active requests, in-flight Bull rows, and pending terminal outboxes are zero, with one historical row in `ignoredHistory`.
5. **Completed locally 2026-08-23:** ADR-19 runs one mandatory DSH sidecar with official igrep and no native/legacy fallback. The signed probe proves BFF/SSE/DB/Scene/recall/wake/no-memory/outbox/Main/cleanup; authenticated full readiness proves both profiles and all bridges. LTX Job `cmt5gl1zz008qhql7xt8dgfu9` and H3 Job `cmt5gt85x008yhql73a8xmcjx` completed the real Deluxe product path, delivered one MediaAsset each, and each produced exactly one 100-Dreamcoin spend Settlement. Direct and persistence gate checks for both routes pass.
6. Configure and prove the in-scope external envelope: production HTTPS and secrets, Postgres/Redis/prefix, signed Chat BFF, R2/S3 write/sign/read/delete, real Chat/Image/Video/Voice canaries, and four-runtime Sentry canary ingest/query. Payment, age verification and compliance remain excluded from this run; `MODERATION_PROVIDER=mock` is intentional and is not a launch gap.
7. In the exact production-like revision, rerun the remaining numbered Chrome customer/operator journeys in `docs/product-audits/2026-08-11-full-product-launch-audit/`, including Premium explicit-model selection, refresh recovery, keyboard/focus and 390/768/1280 layouts. Voice candidate→activate→entitled Chat audio persistence and the approved disposable Account Deletion grace/terminal canary are now signed local-runtime journeys; production must still repeat them against its own release and data authorities. Unexpected console error and network 4xx/5xx must be zero.
8. Run production backfills to zero mismatch, fixed read/write canaries, legacy-traffic drain, error-budget observation window, and independent Product/Engineering/Release signoff.
...
- `docs/product/STRUCTURAL_DEBT_EXECUTION_PLAN.md`: the structural-debt execution record — local service decomposition, Admin v2 Image Library, behavior tests, authenticated browser evidence, and dev CHECK constraints are complete; production DB execution and canary remain project release gates. Updated 2026-08-04 from baseline `3c6e88687`.
...
The repository implementation has passed the recorded database, static, full-test, E2E, immutable-build, PM2, HTTP, local-browser, and three-layer backup/restore portions of the controlled-beta gate at their documented revisions. Because the latest source changes are not yet deployed under one immutable release and the complete authenticated Chrome suite has not been rerun, this is not a current-revision or public-launch claim.
...
Future public launch still requires `bun run check:launch -- --launch-env-file .tmp/production-main.env --admin-env-file .tmp/production-admin.env --chat-env-file .tmp/production-chat.env --gen-env-file .tmp/production-gen.env --report .tmp/check-launch.json --json` to pass against the exact deployed service authorities and real production providers.
...
**落地**：`CreateWorkspace.tsx` 重写为 5 步向导（Identity→Appearance→Personality→Preview→Publish），保留既有 draft API 契约（createDraft→分步 PATCH(step)→preview→submit）；每步推进 autosave 到 draft，并以 localStorage 持久化向导状态实现刷新续编（draft API 无 GET，故走客户端持久化）；Preview 步有 idle/generating/complete/failed 态；Publish 区分 private(approved)/unlisted/public(pending_review，文案明确「通过审核后进入发布准备，Release 发布后上线」）；新增 18+/禁止内容校验文案与 name/age 校验，且当前 step 校验反馈以 polite live region 暴露。E2E (`ui-workflows.e2e.ts`) 覆盖 private success、age validation/live-region status、refresh resume、preview failure recovery、public pending_review submit；Chrome screenshot `51-create-public-review-my-ai.png` 验证 public submit 后 My AI `PENDING REVIEW` 可见；2026-07-04 Chrome Create validation status audit 复验 invalid age 的 `role="status"` / `aria-live="polite"`。
...
For the current `core` scope, this phase is blocked on 24 Gate failures: 23 production-envelope checks for public HTTPS and production secrets, Redis/BullMQ prefix, non-mock R2/S3, service signing/model tokens, and Sentry browser plus four-runtime evidence, plus source-revision authority while the four Sentry reports are absent. Payment and age verification are outside this run.

1. Full E2E against production-like services.
2. Chrome smoke on main/admin.
3. Admin moderation runbook rehearsal.
4. Production service-token separation and signed BFF/model canaries.
5. Storage write/sign/read/delete and retained recovery-version test.

Exit criteria:

- Product, Engineering, and Release sign off.
