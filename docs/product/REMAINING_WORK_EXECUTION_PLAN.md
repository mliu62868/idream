...
The 2026-07-11 Admin correctness review found that several Admin surfaces were interactively covered without certified state/metric semantics. The implemented foundation now includes shared v2 contracts/permissions, migration bootstrap, Generation/Creative truth containment, idempotent command/event reliability, durable chat/main/gen ACK seams, authoritative command acceptance, reconstructable Today queues, a fail-closed and source-decoupled Admin HTTP BFF, and an executable §19.4 cutover-invariant ledger that blocks on nonzero or unavailable checks. Generation Attempt truth now includes attempt-linked append-only canonical events with ordered non-terminal facts, a database-enforced single terminal scope, immutable payload/hash, transactional local-finalize writes, durable manifest ingest, Creative retry queue facts, and evidence-only dry-run/apply keyset backfill; ambiguous legacy terminal rows remain partial/manual rather than fabricated. Character/Release/Visual now has resumable backfill/reconciliation, exact immutable evidence, schedule/publish/rollback executors, Serving/Audit/Outbox/monitor atomicity, a recoverable command worker, V1 state adapter delegation, immutable Chat Session Release pins, explicit verified next-turn migration, derived route-stale dispatch, and canonical 24h/72h monitor collection. Character Performance/Portfolio now has exact content-version/release/placement 7d/28d contracts and read APIs, server-side search/filter/cursor, version change markers, evidence-backed five-state Portfolio DecisionRecords, exposure-chain projection, partial-history backfill/reconciliation, and audited-economics fail-closed semantics; because captured payment/refund/credit attribution authority does not yet exist, contribution margin intentionally remains null/invalid instead of estimated. Creative Retry/Generation now has attempt-level retry executors, durable dispatch, lease/result verification, immutable review decisions, distribution-owned placement publishing, current-slot verification, and v2 lineage/read APIs; release-owned placement remains under Character Release authority. Incident, Review Case, Support/Billing Case, Customer 360, and Character/Creative Today roots now have authoritative aggregation/backfill, assignment/SLA, Evidence/Decision/Action, recovery verification/override, resolve/close, watch/pin/snooze, and scoped server queries; billing mutations remain delegated to Billing authority. Metrics now adds typed chat exchange provenance, generation delivery/AiUsage events, canonical facts, a versioned Registry, golden cohort calculations, fail-closed quality gates, real experiment assignment/exposure, and mature exposed-cohort directional QCE lift. NS-01 remains open: WPCU is official and WSCU/WSCrU/WPSCU remain shadow. Remaining work includes production attempt/authority/Portfolio backfills and two mature shadow windows; production Character monitor/canary and exposure/journey/D7 coverage; captured-payment/refund/credit attribution authority; responsive Character/Creative UI browser validation; provider-native billing dispute source; comments/mentions/handoff/saved views; experiment management and guardrail/statistical evaluation; legacy traffic zero, canary, and observation gates. UI availability, additive schema, calculable local metrics, command acceptance, or one vertical slice alone must not be counted as final cutover. The remediation plan above remains the execution SSoT.
