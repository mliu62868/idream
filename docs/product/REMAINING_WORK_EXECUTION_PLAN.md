...
The 2026-07-11 Admin correctness review found that several Admin surfaces were interactively covered without certified state/metric semantics. The local implementation now closes the code-owned scope: shared v2 contracts/permissions; source-decoupled fail-closed Admin BFF; reconstructable Today/search/SavedViews; official Character creation and immutable Release/Serving lifecycle; Creative review/placement; Generation Request/Attempt/Transport/Artifact/Delivery/Settlement and idempotent cancellation; Incident detect/assign/action/verify/split/merge/postmortem-close; typed Case wait/reopen/recurrence and Incident linking; Customer 360; canonical events/facts/metrics/experiments; atomic approval/Command/Audit/Outbox; database-enforced immutable evidence; executable §19.4 reconciliation and SLO readiness. Local PostgreSQL shadow is `0 violation / 0 unavailable / 0 mismatch`; 100k Case + 100k Job + 1m Event load/replay and fresh/repeat/app-compatible migration rehearsal pass; all 998 tests plus lint/typecheck/production build pass. A current double-service Playwright v2 Gate passes 4/4 at desktop and 375px across Character create/resume; Creative review→placement→live verification; Incident recovery verification→resolve→postmortem/close; Case evidence→decision→verification→close; Today Recently resolved projection/deep links; URL/keyboard/filtered-empty state; authoritative database facts; zero console errors and zero page overflow. The current legacy/Admin compatibility Playwright gate also passes 25/25 after separating Team Access from Customer 360 and retaining direct-link Support/Moderation/Risk command parity outside primary navigation until equivalent Case commands exist. A browser performance baseline is still pending the benchmark tool's one-time local setup confirmation. NS-01 remains open: WPCU is official and WSCU/WSCrU/WPSCU remain shadow. Remaining work is external/production evidence and authority: production backfills; real Character/exposure/journey/D7 and payment attribution coverage; provider-native dispute facts; canonical experiment guardrails and two mature shadow windows; production browser/API baseline; read/write canary; legacy traffic zero; sustained error-budget observation; Product/Data/Design/Ops/Release DRI sign-off. Local code, fixture data, or one-time smoke results must not be counted as final cutover. The remediation plan above remains the execution SSoT.
