...
The 2026-08-11 full-product audit does not change that classification. Current hardened local launch readiness under the fixed `LAUNCH_SCOPE=core` contract is `22 pass / 37 fail / 1 warn` (`.tmp/check-launch-2026-08-12-final-core.json`); the executable gate excludes only payment and age verification, rejects unknown scope values, and compliance remains outside this run's verdict. The gate verifies exact repository/database migration authority, fails closed on the exact Main→Chat failed durable backlog before production PM2 may mutate queues or processes, directly verifies the checksummed Main PostgreSQL + Chat FS + Blob source/isolated-restore bundle against the repository migration revision, requires workflow-bound immutable TerminalRecord evidence for enabled image/video generation, and separately verifies the latest Image/Video Main persistence chain through Job/Attempt/receipt/outbox/artifact/delivery/MediaAsset. The original 48 target-missing Main→Chat carriers were terminalized through the dual-permission Chrome operator flow; the read-only audit is now zero, Chat stores 48 `discarded_target_missing` receipts, Main stores 48 matching audits, and the backlog check passes. The historical migration-60 bundle passes its checksum/equality proof but is correctly rejected as `60/71`; no current migration-71 bundle is configured. The stale Premium v1 makes Product Config fail, so the gate also correctly refuses to accept the otherwise successful standalone Image/Video probes as current product-configuration authority. Disposable PostgreSQL 16 rehearsals prove both fresh 71/71+seed and legacy Premium v1→RedMix3 v2 upgrade states; archived v1 pins are preserved, active v2 is executable, and Product Config is green. The checker also fails PostgreSQL 14 in a structured way before reading PG16-only catalog fields. None of that changes the product database's 67/71 state. The two former pipeline-credential failures and pipeline-model warning were false attribution from `GEN_IMAGE_PROVIDER=mock` and have been removed; mock still fails the real provider and live-probe checks. Sentry runtime wiring and a capture/flush/query gate now exist, but no real Sentry canary was executed. Local RedMix3 Image, LTX Video, Fish Voice, dual-customer Chrome, customer-to-Release/Serving and operator journeys now have current evidence. Public launch remains blocked by four unapplied operator migrations, the in-scope production envelope, current restore evidence and release observation/signoff.

Account Deletion's code-owned rollback boundary is closed with `user.account_deletion.requested.v2`: the event has a dedicated current-Chat ingest path, stays pending on mixed-version 404s instead of exhausting into `failed`, and is absent from the rollback Main sender whitelist. The 71-migration fresh/upgrade/redeploy/rollback rehearsal passes, but the product database still lacks that migration, so this is deployment evidence—not a completed production deletion canary.

### 2026-08-11 active closure queue

1. Deploy the fail-closed admission/runtime code first. An authorized operator then reviews and applies, in order, `20260811133000_redmix3_public_profile_cutover`, `20260811143000_runtime_schema_reconciliation`, `20260811160000_voice_clip_scene_payload_authority`, and `20260811190000_account_deletion_terminal_authority`. Rerun migration authority and Product Config; require 71/71, archived legacy Premium v1, active Premium RedMix3 v2, exact Voice constraint and Account Deletion tables before admitting traffic.
2. **Completed 2026-08-11:** the read-only audit proved all 48 envelopes were payload-valid target-missing callbacks with no Chat receipt, receiver attachment, source outbox or replayable effect. Admin Chat Operations → `Record target missing` persisted 48 Chat `discarded_target_missing` receipts before Main CAS/audit terminalization and explicitly recorded no user effect. The post-action read-only audit is `ok=true`, failed/replayable/reconciliation-required are all zero, and the launch backlog check passes.
3. **Code-owned entry completed 2026-08-12:** `bun run recovery:rehearse` now provides a read-only sanitized plan and a typed-confirmation apply path that refuses live runtime, non-exact migrations, active clients, durable work, split authorities, unversioned remote objects and overwrite. It performs PostgreSQL 16 isolated restore, Chat FS byte restore, local Blob restore or exact-version R2/S3 copy/read/cleanup, verifies the produced v1 bundle with the same launch authority, and atomically publishes only on equality. After the four migrations are operator-applied, an authorized operator must still run it against the real quiesced migration-71 environment, set `RECOVERY_REHEARSAL_BUNDLE`, establish disk-capacity/retention alerts and retain the independent restore evidence. The 2026-07-18 60-migration backup remains historical evidence only.
4. Configure and prove the in-scope external envelope: production HTTPS and secrets, Postgres/Redis/prefix, signed Chat BFF, R2/S3 write/sign/read/delete, real Chat/Image/Video/Voice canaries, and four-runtime Sentry canary ingest/query. Payment, age verification and compliance remain excluded from this run; `MODERATION_PROVIDER=mock` is intentional and is not a launch gap.
5. In the exact production-like revision, rerun the numbered Chrome customer/operator journeys in `docs/product-audits/2026-08-11-full-product-launch-audit/`, including Premium explicit model, Voice candidate→activate→Chat audio persistence, Account Deletion grace/terminal canary on an approved disposable actor, refresh recovery, keyboard/focus and 390/768/1280 layouts. Unexpected console error and network 4xx/5xx must be zero.
6. Run production backfills to zero mismatch, fixed read/write canaries, legacy-traffic drain, error-budget observation window, and independent Product/Engineering/Release signoff.
