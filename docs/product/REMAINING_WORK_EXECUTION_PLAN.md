...
The 2026-07-11 Admin correctness review found that several Admin surfaces were interactively covered without certified state/metric semantics. The local implementation now closes the principal code-owned authority paths: shared v2 contracts/permissions; source-decoupled fail-closed Admin BFF; reconstructable Today/search/SavedViews; official Character creation and immutable Release/Serving lifecycle; Creative review/placement; Generation Request/Attempt/Transport/Artifact/Delivery/Settlement and idempotent cancellation; Incident detect/assign/action/verify/split/merge/postmortem-close; typed Case wait/reopen/recurrence and Incident linking; Customer 360; canonical events/facts/metrics/experiments; atomic approval/Command/Audit/Outbox; database-enforced immutable evidence; executable §19.4 reconciliation and SLO readiness. The schema-v5 final Gate now requires immutable digested artifacts signed by trusted collectors, six independent role-bound DRI approvals of one canonical digest, a final release envelope, fixed representative read/write canaries with Command/Audit/Outbox authority, two zero-legacy cycles and the mature observation window. Trust comes only from an external public-key registry; release-key possession alone cannot create collector or DRI authority. Remaining work is external/production evidence and authority: provision the release, collector and six DRI key material plus independent trust registry outside the repository; production backfills and zero-mismatch shadow on a dedicated snapshot; real Character/exposure/journey/D7 and payment attribution coverage; provider-native dispute facts; canonical experiment guardrails and two mature shadow windows; production browser/API baseline; execute and immutably collect the fixed production canaries; legacy traffic zero; sustained error-budget observation; independent Product/Data/Design/Ops/Release DRI approval. Local code, fixture data, unsigned manifests, one-time smoke results, or release-key-only envelopes must not be counted as final cutover. The remediation plan above remains the execution SSoT.
