...
The 2026-07-11 Admin correctness review found that several Admin surfaces were interactively covered without certified state/metric semantics. The local code implementation now includes shared v2 contracts/permissions, migration bootstrap, Generation/Creative truth containment, idempotent command/event reliability, durable chat/main/gen ACK seams, authoritative command execution, reconstructable Today queues, a fail-closed and source-decoupled Admin HTTP BFF, and an executable §19.4 cutover-invariant ledger that blocks on nonzero or unavailable checks. Generation Attempt truth has attempt-linked append-only canonical events, ordered non-terminal facts, a database-enforced single terminal scope, immutable payload/hash, transactional local-finalize writes, durable manifest ingest, Creative retry queue facts, and evidence-only resumable backfill; ambiguous legacy terminal rows remain partial/manual rather than fabricated. Character/Release/Visual has exact immutable evidence, schedule/publish/rollback executors, Serving/Audit/Outbox/monitor atomicity, recoverable commands, immutable Chat Session Release pins, explicit migration, route-stale dispatch, and 24h/72h monitoring. Character Performance/Portfolio and Creative Review/Placement have dedicated responsive operator workspaces and evidence-backed authority flows. Incident, Review Case, Support/Billing Case, Customer 360, and Character/Creative Today have authoritative roots, assignment/SLA, Evidence/Decision/Action, recovery verification, collaboration timelines, watching, and scoped SavedViews. Managed Experiments has immutable definitions, assignment/exposure authority, start/stop, mature exposed-cohort analysis, confidence intervals/p-values, and fail-closed maturity/quality/guardrail gates. The Admin UI and BFF have been exercised on desktop and 375px mobile with URL reload recovery and a clean browser console. NS-01 remains open: WPCU is official and WSCU/WSCrU/WPSCU remain shadow. The remaining work is production evidence and external authority: production attempt/authority/Portfolio backfills; production Character monitor/canary and exposure/journey/D7 coverage; captured-payment/refund/credit attribution; provider-native billing dispute facts; canonical experiment guardrail facts and two mature shadow windows; legacy traffic zero; performance/load/chaos, error-budget, canary, and sustained observation gates. Local UI availability, additive schema, calculable metrics, or passing fixture data must not be counted as final cutover. The remediation plan above remains the execution SSoT.
