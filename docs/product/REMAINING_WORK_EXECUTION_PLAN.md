...
The 2026-07-11 Admin correctness review found that several Admin surfaces are interactively covered but do not yet have certified state/metric semantics. The first implementation batch covers metric truth containment, shared v2 contracts/permissions, Generation/Creative truth, migration bootstrap, additive authority models, command/event idempotency, durable receipt/outbox primitives plus a minimal production chat↔main durable ACK and gen completion-manifest vertical slice, fail-closed lease recovery, v2 command status/acceptance and a reconstructable Today projection. The second Character vertical slice adds resumable Character/Release/Visual backfill and reconciliation, exact Visual/Reference evidence, schedule/publish/rollback execution, Serving pointer/Audit/Outbox/monitor atomicity, a command worker, and V1 state adapter delegation. Remaining Admin work includes the Character Project/preview/Portfolio and Chat pin surfaces, route-qualification evaluation/stale invalidation, monitor fact collection, other domain backfills/executors and verification, remaining Character/Creative roots in Today, canonical metrics, and broad migration of legacy direct queue producers. UI availability, additive schema, or one vertical slice alone must not be counted as final cutover. The remediation plan above supersedes older completion assumptions and remains the execution SSoT.
