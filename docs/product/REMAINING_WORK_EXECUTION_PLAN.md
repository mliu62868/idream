...
The 2026-07-11 Admin correctness review found that several Admin surfaces were interactively covered without certified state/metric semantics. The local implementation now closes the principal code-owned authority paths: shared v2 contracts/permissions; source-decoupled fail-closed Admin BFF; reconstructable Today/search/SavedViews; official Character creation and immutable Release/Serving lifecycle; Creative review/placement; Generation Request/Attempt/Transport/Artifact/Delivery/Settlement and idempotent cancellation; Incident detect/assign/action/verify/split/merge/postmortem-close; typed Case wait/reopen/recurrence and Incident linking; Customer 360; canonical events/facts/metrics/experiments; atomic approval/Command/Audit/Outbox; database-enforced immutable evidence; executable §19.4 reconciliation and SLO readiness. The current full suite is 1167 tests across Shared/Main/Admin/Chat/Gen, with lint, typecheck and production build passing. The production-table-shaped load harness passes at 100k Case + 100k Job + 1m Event scale using the production indexes unchanged, and fresh/repeat/previous-app/forward-fix migration rehearsal passes. Shadow is deliberately not claimed green: the current shared test database reports `410 violations / 352 mismatches` and blocks decision use because seed/test/legacy rows have not undergone a dedicated cutover backfill. A current double-service Playwright v2 Gate passes 9/9 at desktop, tablet, and 375px across Character create/resume and immutable release rollback; Creative/Incident/Case loops; Today and Jobs canonical deep links; retry conflict/focus; WCAG checks; URL/filtered-empty/responsive state. The legacy/Admin compatibility Playwright gate passes 25/25. The schema-v4 final Gate now requires a domain-separated Ed25519 signature from an independently trusted release key before it evaluates the seven-day production window, fixed representative read/write canaries with Command/Audit/Outbox authority, two zero-legacy cycles and DRI sign-offs; arbitrary JSON or a trivial single-request smoke can no longer self-assert production provenance. Independent BFF read/write kill switches provide fail-closed cutover control. NS-01 remains open: WPCU is official and WSCU/WSCrU/WPSCU remain shadow. Remaining work is external/production evidence and authority: provision the release signing/verifying key material outside the repository; production backfills and zero-mismatch shadow on a dedicated snapshot; real Character/exposure/journey/D7 and payment attribution coverage; provider-native dispute facts; canonical experiment guardrails and two mature shadow windows; production browser/API baseline; execute the fixed production read/write canaries; legacy traffic zero; sustained error-budget observation; Product/Data/Design/Ops/Release DRI sign-off. Local code, fixture data, unsigned manifests, or one-time smoke results must not be counted as final cutover. The remediation plan above remains the execution SSoT.
