...
The repository, automated journeys, and current service probes prove a working local MVP implementation. The code-owned core chains are closed, but the final revision has not received one complete authenticated Chrome rerun and public launch is not ready. After the 2026-06-26 scope decision, the active milestone remains **internal demo / controlled beta**, not public launch.

The continuation keeps the public-launch classification unchanged while closing the local Chat, Recovery, Admin-text and destructive browser evidence stop-ships. The latest structured report is `.tmp/check-launch-2026-08-14-final-user-journeys.json`: `44 pass / 23 fail / 0 warn / 67 total` under `LAUNCH_SCOPE=core`; payment and age verification are excluded. It records `generatedAt`, the expected source revision, the normalized probe-evidence digest and a secret-masked environment digest. Main, Admin, Chat, and Gen env authorities are resolved independently; APP_ENV, internal/BFF authority, BullMQ prefix and immutable source revision cannot drift across services. Current signed Chat, Chat model, Image, Video, Voice, Admin text and persistence reports are bound to that revision. Four package-specific Sentry probes bind their honest local failures to the same revision, so source-revision authority passes while the external Sentry canary remains red. Main migration authority remains exact `71/71`; target-missing carriers remain terminalized; failed/replayable/reconciliation-required backlog is zero; Image 2/2 and Video 1/1 ownership is green after restart. Public launch remains blocked by 23 production URL/secret/Redis/Blob/Sentry envelope checks.
