...
export function PricingWorkspace({ canWrite }: { canWrite: boolean }) {
  ...
  function confirmVersionAction(row: PricingRecord, action: "publish" | "rollback") {
    ...
    setConfirmation({
      ...
      summary: <span>{name} · version {display(row.version)}</span>,
  ...
  return (
    <section aria-labelledby="pricing-workspace-title" className="space-y-5">
      <div id="pricing-workspace-title"><PageHeader purpose="Version, publish, and roll back customer-facing generation prices while keeping every decision auditable." title="Pricing & Offers" /></div>
      <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-[var(--ad-text-muted)]" role="status"><span>Legacy compatibility authority · freshness watermark unavailable{refreshedAt ? <> · refreshed <time dateTime={refreshedAt}>{new Date(refreshedAt).toLocaleTimeString()}</time></> : null}</span>{!canWrite ? <strong>Read only · config.pricing.write is not granted</strong> : null}</div>
      ...
      <form className="grid gap-3 rounded-lg border border-[var(--ad-border)] bg-[var(--ad-surface)] p-4 md:grid-cols-2 xl:grid-cols-[minmax(240px,1fr)_180px_180px_auto]" onSubmit={apply}>
        <Field label="Search pricing authority" onChange={(search) => setQueryDraft((current) => ({ ...current, search }))} placeholder="rule key, label, or ID" value={queryDraft.search} />
        ...
        <div className="flex items-end gap-2"><button className="min-h-11 rounded-md bg-[var(--ad-ink)] px-4 text-sm font-semibold text-white" type="submit">Apply</button>{filtered ? <button aria-label="Clear pricing filters" className="grid min-h-11 min-w-11 place-items-center rounded-md border border-[var(--ad-border)]" onClick={clearFilters} type="button"><X className="h-4 w-4" /></button> : null}</div>
      </form>
...
function PricingLoading() { return <div aria-label="Loading pricing authority" className="overflow-hidden rounded-lg border border-[var(--ad-border)] bg-[var(--ad-surface)]" role="status"><span className="sr-only">Loading pricing authority</span>{[0, 1, 2, 3].map((row) => <div className="grid min-h-14 animate-pulse grid-cols-5 gap-4 border-b border-[var(--ad-border)] px-4 py-3 last:border-0" key={row}>{[0, 1, 2, 3, 4].map((cell) => <span className="h-4 rounded bg-black/5" key={cell} />)}</div>)}</div>; }
