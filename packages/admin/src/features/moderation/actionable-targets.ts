...
//         有注释说明这是有意的），而裁定端 `applyModerationAction`
//         （`admin-v2/moderation/moderation-effect.ts`）只实现了下面这几类，其余一律
//         `throw Errors.badRequest("Unsupported moderation target type")`。
...
// INVARIANT: 这份清单必须和 moderation-effect.ts 的分支逐字一致。多写一个，就是把审核员
//            送回那个 400；少写一个，就是把本来能收口的活藏起来。

export const ACTIONABLE_REPORT_TARGET_TYPES = [
  "character",
  "media",
  "feed_item",
  "comic",
...
/** 队列「目标类型」筛选：主站各举报入口实际写入的值（空串 = 全部）。 */
export const REPORT_TARGET_TYPE_FILTERS = [
  "",
  ...ACTIONABLE_REPORT_TARGET_TYPES,
  "user_profile",
  "chat_message",
];
