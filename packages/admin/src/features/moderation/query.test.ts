...
describe("moderation query", () => {
  it("restores all three authority cursors and maps filters to scoped server queries", () => {
    ...
    expect(moderationQueuePath(query, "reports")).toBe("/api/v1/admin/moderation/queue?scope=reports&limit=25&search=case&status=open&targetType=media&reportCursor=r");
    expect(moderationQueuePath(query, "media")).toBe("/api/v1/admin/moderation/queue?scope=media&limit=25&search=case&status=open&targetType=media&mediaCursor=m");
    expect(moderationQueuePath(query, "appeals")).toBe("/api/v1/admin/moderation/queue?scope=appeals&limit=25&search=case&status=open&targetType=media&appealCursor=a");
  });
