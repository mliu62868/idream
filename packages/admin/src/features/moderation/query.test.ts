...
describe("moderation query", () => {
  it("restores all three authority cursors and maps filters to scoped server queries", () => {
    ...
    expect(moderationQueuePath(query, "appeals")).toBe("/api/v1/admin/moderation/queue?scope=appeals&limit=25&search=case&status=open&targetType=media&reportCursor=r&mediaCursor=m&appealCursor=a");
  });
