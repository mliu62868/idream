...
function ProfileDetail({ canWrite, jobs, onConfirm, onTest, profile, review, setReview, setTestPrompt, testBusy, testPrompt }: {
  ...
  const dryRun = object(profile.dryRunSummary);
  return <section className="space-y-4 rounded-lg border border-[var(--ad-border)] bg-[var(--ad-surface)] p-4">
    ...
    {canWrite ? <div className="flex flex-wrap gap-2">
      {status === "draft" ? <Action icon={<Activity className="h-4 w-4" />} label="Dry Run" onClick={() => onConfirm({ title: `Dry run profile ${id}`, endpoint: `/api/v1/admin/generation/model-profiles/${id}/dry-run`, method: "POST", expected: id, payload: (reason) => ({ reason }) })} /> : null}
...
function object(value: unknown) { return typeof value === "object" && value !== null && !Array.isArray(value) ? value as Record<string, unknown> : {}; }
