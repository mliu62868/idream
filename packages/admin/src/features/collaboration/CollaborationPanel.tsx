  ...
  return (
    <section aria-labelledby={`collaboration-${targetType}-${targetId}`} className="rounded-xl border border-[var(--ad-border)] bg-[var(--ad-surface)] p-4">
      ...
      <p className="mt-2 break-words text-xs text-[var(--ad-text-muted)]">
        {watcherIds.length === 0 ? t("No watchers") : t("{count} watchers: {ids}", { count: watcherIds.length, ids: watcherIds.join(", ") })}
      </p>
      ...
      {canWrite ? (
        <form className="mt-3 grid gap-3 rounded-lg bg-[var(--ad-surface-subtle)] p-3" onSubmit={(event) => { event.preventDefault(); void submit(); }}>
          <div className="grid gap-3 sm:grid-cols-[160px_1fr]">
            <label className="grid gap-1 text-xs font-semibold text-[var(--ad-text-muted)]">{t("Activity type")}<select className={fieldClass} onChange={(event) => setKind(event.target.value as ActivityKind)} value={kind}>{Object.entries(kindLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
            <label className="grid gap-1 text-xs font-semibold text-[var(--ad-text-muted)]">{t("Mention actor IDs")}<input className={fieldClass} onChange={(event) => setMentions(event.target.value)} placeholder={t("user-id-1, user-id-2")} value={mentions} /></label>
          </div>
          {kind === "handoff" ? <label className="grid gap-1 text-xs font-semibold text-[var(--ad-text-muted)]">{t("Handoff actor ID")}<input className={fieldClass} onChange={(event) => setHandoffActorId(event.target.value)} required value={handoffActorId} /></label> : null}
      ...
      <div className="mt-4">
        ...
        {loading && items.length === 0 ? <p className="mt-3 text-sm text-[var(--ad-text-muted)]" role="status">{t("Loading collaboration activity…")}</p> : items.length === 0 ? <p className="mt-3 rounded-md bg-[var(--ad-surface-subtle)] p-3 text-sm text-[var(--ad-text-muted)]">{t("No activity yet. The first comment or handoff will appear here.")}</p> : (
          <ol className="mt-3 space-y-3">
            {items.map((activity) => <li className="border-l-2 border-[var(--ad-border)] pl-3" key={activity.id}><div className="flex flex-wrap items-center justify-between gap-2"><span className="text-xs font-semibold">{t(activity.kind.replaceAll("_", " "))} · <span className="font-mono">{activity.actorId}</span></span><time className="text-xs text-[var(--ad-text-muted)]" dateTime={activity.createdAt}>{new Date(activity.createdAt).toLocaleString()}</time></div>{activity.body ? <p className="mt-1 whitespace-pre-wrap break-words text-sm leading-6">{activity.body}</p> : null}{activity.mentionedIds.length > 0 ? <p className="mt-1 break-words text-xs text-[var(--ad-text-muted)]">{t("Mentions:")} {activity.mentionedIds.map((id) => `@${id}`).join(", ")}</p> : null}{activity.metadata.handoffToActorId ? <p className="mt-1 text-xs">{t("Handoff to:")} <span className="font-mono">{activity.metadata.handoffToActorId}</span></p> : null}{activity.metadata.checklistItems.length > 0 ? <ul className="mt-2 space-y-1 text-xs">{activity.metadata.checklistItems.map((item) => <li key={item.id}>{item.completed ? "✓" : "○"} {item.label}{item.ownerId ? ` — ${item.ownerId}` : ""}</li>)}</ul> : null}{activity.metadata.attachments.length > 0 ? <p className="mt-2 break-words text-xs text-[var(--ad-text-muted)]">{t("Evidence:")} {activity.metadata.attachments.map((item) => item.label).join(", ")}</p> : null}</li>)}
          </ol>
        )}
