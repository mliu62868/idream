...
export function DeadLetterWorkspace({ canWrite }: { canWrite: boolean }) {
  ...
  function requestAction(input: {
    ...
    setConfirmation({
      title: input.title,
      summary: <span>{input.ids.length} generation request{input.ids.length === 1 ? "" : "s"} · authority confirmation required</span>,
  ...
  return (
    <section aria-labelledby="dead-letter-workspace-title" className="space-y-5">
      <div id="dead-letter-workspace-title">
        <PageHeader purpose="Triage failed and blocked generation requests, then requeue or discard them through audited authority commands." title="Dead-letter Queue" />
      </div>
      ...
      <form className="grid gap-3 rounded-lg border border-[var(--ad-border)] bg-[var(--ad-surface)] p-4 md:grid-cols-2 xl:grid-cols-[minmax(260px,1fr)_160px_160px_220px_auto]" onSubmit={apply}>
        ...
        <div className="flex items-end gap-2">
          ...
          {filtered ? <button aria-label="Clear dead-letter filters" className="grid min-h-11 min-w-11 place-items-center rounded-md border border-[var(--ad-border)]" onClick={() => navigate(defaultDeadLetterQuery)} type="button"><X className="h-4 w-4" /></button> : null}
        </div>
      </form>
        ...
        <>
          <div className="flex flex-wrap items-center gap-3 rounded-lg border border-[var(--ad-border)] bg-[var(--ad-surface)] px-4 py-3">
            <label className="flex items-center gap-2 text-xs text-[var(--ad-text-muted)]">
              <input aria-label="Select all dead-letter jobs" checked={allSelected} onChange={() => setSelected(allSelected ? new Set() : new Set(rowIds))} type="checkbox" />
              Select all
            </label>
          ...
          {rows.length === 0 ? <EmptyState action={filtered ? <button className="min-h-11 rounded-md border border-[var(--ad-border)] px-4 text-sm font-semibold" onClick={() => navigate(defaultDeadLetterQuery)} type="button">Clear filters</button> : undefined} hint={filtered ? "The complete dead-letter authority query returned no matches." : "No failed or blocked generation requests require triage."} title={filtered ? "No dead-letter jobs match these filters" : "No dead-letter jobs"} /> : (
            <div aria-label="Dead-letter Queue scrollable table" className="overflow-x-auto rounded-lg border border-[var(--ad-border)] bg-[var(--ad-surface)]" role="region" tabIndex={0}>
              <table className="w-full min-w-[980px] text-left text-sm">
                <caption className="sr-only">Dead-letter Queue</caption>
                ...
                <tbody>{rows.map((row) => {
                  ...
                  return <tr className="border-b border-[var(--ad-border)] last:border-0" key={id}>
                    <td className="px-3 py-3"><input aria-label={`Select dead-letter job ${id}`} checked={selected.has(id)} onChange={() => toggle(id)} type="checkbox" /></td>
...
function AuthorityError({ hasSnapshot, message, onRetry }: { hasSnapshot: boolean; message: string; onRetry: () => void }) {
  return <div className="rounded-md bg-[var(--ad-red-bg)] p-3 text-sm text-[var(--ad-red-text)]" role="alert">Dead-letter authority refresh failed: {message}<button className="ml-3 min-h-8 rounded border border-current px-2 font-semibold" onClick={onRetry} type="button">Retry dead-letter</button>{hasSnapshot ? <span className="ml-2">The last good snapshot remains visible.</span> : null}</div>;
}

function Loading() {
  return <div aria-label="Loading dead-letter authority" className="space-y-3 rounded-lg border border-[var(--ad-border)] bg-[var(--ad-surface)] p-4" role="status"><span className="inline-flex items-center gap-2 text-sm text-[var(--ad-text-muted)]"><Loader2 className="h-4 w-4 animate-spin" />Loading dead-letter authority</span></div>;
}
