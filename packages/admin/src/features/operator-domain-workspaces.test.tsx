...
describe("Character and Creative operator workspaces", () => {
  it("renders explicit effective-permission denial instead of attempting a hidden write", () => {
    const html = renderToStaticMarkup(<CharacterWorkspace permissions={{ read: false, writeProject: false, publishRelease: false }} view={{ kind: "list" }} />);
  ...
  it("renders server-authority search controls while portfolio data is loading", () => {
    const html = renderToStaticMarkup(<CharacterWorkspace permissions={{ read: true, writeProject: true, publishRelease: true }} view={{ kind: "list" }} />);
