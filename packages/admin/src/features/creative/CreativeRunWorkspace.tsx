...
function CreateRunForm({
  ...
  return (
    <section className="mt-5 rounded-xl border border-[var(--ad-border)] bg-[var(--ad-surface)] p-4 sm:p-5" aria-labelledby="create-creative-run-title">
            ...
            <select className={`${fieldClass} mt-1`} disabled={Boolean(createIntent)} onChange={(event) => chooseProfile(event.target.value)} value={profileId}>
              {(options?.profiles ?? []).map((profile) => (
                <option key={`${profile.profileKey}:${profile.profileVersion}`} value={profile.profileKey}>
                  {profile.label}{profile.recommended ? ` · ${t("Recommended")}` : ""}
                </option>
              ))}
            </select>
          </label>
      ...
      {createIntent?.committedTargetId ? <p className="mt-3 text-sm" role="status">Created Run receipt: <Link className="font-medium underline underline-offset-4" href={`/admin/creative/runs/${createIntent.committedTargetId}`}>open {createIntent.committedTargetId}</Link></p> : null}
...
function AssetViewer({ run, selected, onSelect }: { run: CreativeRunDetail; selected: number; onSelect: (index: number) => void }) {
  ...
  return <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_280px]" onKeyDown={move} tabIndex={0} aria-label="Creative asset viewer. Use left and right arrow keys to move between items."><div className="min-h-80 overflow-hidden rounded-xl border border-[var(--ad-border)] bg-black/[0.04]">{item.asset ? (
    ...
    <img alt={`Creative item ${item.ordinal + 1}`} className="max-h-[70vh] w-full object-contain" src={item.asset.url} />
  ) : <div className="grid min-h-80 place-items-center text-[var(--ad-text-muted)]"><ImageIcon className="h-8 w-8" /><span>No valid artifact</span></div>}</div><aside className="rounded-xl border border-[var(--ad-border)] bg-[var(--ad-surface)] p-4"><div className="flex flex-wrap gap-2"><StatusBadge value={item.executionState} /><StatusBadge value={item.status} /><StatusBadge value={item.retryability} /></div><dl className="mt-4 space-y-3 text-xs"><div><dt className="text-[var(--ad-text-muted)]">Request / attempt</dt><dd className="mt-1 break-all">{item.lineage.requestId ?? "Unavailable"}<br />{item.lineage.attemptId ?? "Unavailable"}</dd></div><div><dt className="text-[var(--ad-text-muted)]">Provider request / Comfy prompt</dt><dd className="mt-1 break-all">{item.lineage.providerRequestId ?? "Pending"}</dd></div><div><dt className="text-[var(--ad-text-muted)]">Asset</dt><dd className="mt-1 break-all">{item.asset?.id ?? "Unavailable"}</dd></div><div><dt className="text-[var(--ad-text-muted)]">Latest review</dt><dd className="mt-1">{item.review ? `${item.review.decision} · ${item.review.identityConsistency}` : "Pending"}</dd></div><div><dt className="text-[var(--ad-text-muted)]">Placement</dt><dd className="mt-1">{item.placement ? `${item.placement.slot} · ${item.placement.verificationState}` : "Unplaced"}</dd></div></dl></aside></div>;
}
  ...
  return (
    <section className="mt-5 rounded-xl border border-[var(--ad-border)] bg-[var(--ad-surface)] p-4" aria-labelledby="creative-review-context-title">
      ...
      <dl className="mt-4 grid gap-3 text-xs sm:grid-cols-2 lg:grid-cols-5">
        <div><dt className="text-[var(--ad-text-muted)]">{t("Intended use")}</dt><dd className="mt-1 font-medium">{run.purpose.replaceAll("_", " ")}</dd></div>
        ...
        <div><dt className="text-[var(--ad-text-muted)]">{t("Image route")}</dt><dd className="mt-1 font-medium">{run.reviewContext.profile.label ?? run.reviewContext.profile.key ?? t("Unavailable")}{run.reviewContext.profile.version ? ` · v${run.reviewContext.profile.version}` : ""}</dd></div>
        <div><dt className="text-[var(--ad-text-muted)]">{t("Recipe")}</dt><dd className="mt-1 font-medium">{run.reviewContext.recipe.label ?? run.reviewContext.recipe.key ?? t("Unavailable")}{run.reviewContext.recipe.version ? ` · v${run.reviewContext.recipe.version}` : ""}</dd></div>
...
function ReviewForm({ run, itemIndex, permissions, reload }: { run: CreativeRunDetail; itemIndex: number; permissions: Permissions; reload: () => Promise<void> }) {
  ...
  const [identityConsistency, setIdentityConsistency] = useState<"passed" | "failed" | "unscored">(
    identityReviewMode === "preserves_identity" ? "passed" : "unscored",
  );
  ...
  if (immutableReview) {
    return <section className="mt-4 rounded-xl border border-[var(--ad-border)] bg-[var(--ad-surface)] p-4" aria-labelledby="creative-review-title"><h3 className="font-semibold" id="creative-review-title">{t("Immutable review decision")}</h3><p className="mt-2 text-sm"><strong className="capitalize">{t(immutableReview.decision)}</strong> · {t("identity")} {t(immutableReview.identityConsistency)}{immutableReview.score !== null ? ` · ${immutableReview.score}/100` : ""}</p>{immutableReview.quality ? <ul className="mt-3 grid gap-2 text-xs sm:grid-cols-2">{reviewQualityChecks.map(([key, label]) => <li className="rounded-md bg-black/[0.035] px-3 py-2" key={key}>{t(immutableReview.quality?.[key] ? "Passed" : "Failed")} · {t(label)}</li>)}</ul> : null}<p className="mt-3 text-sm leading-6 text-[var(--ad-text-muted)]">{immutableReview.reason}</p>{immutableReview.supersedesDecisionId ? <p className="mt-2 break-all text-xs text-[var(--ad-text-muted)]">{t("Supersedes")} {immutableReview.supersedesDecisionId}</p> : null}{canWithdrawApproval ? <div className="mt-4 border-t border-[var(--ad-border)] pt-4"><h4 className="text-sm font-semibold">{t("Terminal disposition")}</h4><p className="mt-1 text-xs leading-5 text-[var(--ad-text-muted)]">{t("If this approved candidate will not be activated, record a superseding rejection so every candidate has an explicit terminal outcome and the Run can close.")}</p><p className="mt-2 text-xs text-[var(--ad-text-muted)]">The original score, identity result and visible-quality evidence are preserved. If the asset is selected by a Character authority, replace or withdraw it there first.</p><label className="mt-3 block text-xs font-semibold text-[var(--ad-text-muted)]">{t("Withdrawal reason")}<textarea className={`${textAreaClass} mt-1`} onChange={(event) => setReason(event.target.value)} placeholder={t("Explain why this approved candidate will not be used")} value={reason} /></label>{error ? <p className="mt-3 text-sm text-[var(--ad-red-text)]" role="alert">{error}</p> : null}{warning ? <p className="mt-3 rounded-md bg-[var(--ad-yellow-bg)] px-3 py-2 text-sm text-[var(--ad-yellow-text)]" role="status">{warning}</p> : null}<div className="mt-3"><WorkspaceButton disabled={!permissions.review || busy || !validReason} onClick={() => void decide("rejected")} tone="danger"><X className="h-4 w-4" /> {characterAssetReview ? "Record superseding rejection" : t("Withdraw approval")}</WorkspaceButton></div></div> : immutableReview.decision === "approved" && item.placement ? <p className="mt-4 rounded-md bg-[var(--ad-yellow-bg)] px-3 py-2 text-xs text-[var(--ad-yellow-text)]">{item.placement.status === "scheduled" && item.placement.verificationState === "verifying" ? t("Use Withdraw staged placement below before superseding this approval.") : t("This candidate is already active. Replace its live placement before superseding the approval.")}</p> : null}</section>;
  }
  return <section className="mt-4 rounded-xl border border-[var(--ad-border)] bg-[var(--ad-surface)] p-4" aria-labelledby="creative-review-title"><h3 className="font-semibold" id="creative-review-title">{requiresEvidenceRepair ? "Complete missing review evidence" : "Review decision"}</h3><p className="mt-1 text-xs text-[var(--ad-text-muted)]">{requiresEvidenceRepair ? "The earlier immutable decision is preserved, but it cannot authorize selection without the required visible evidence. This new decision will supersede it." : "Record what you observed. Decision, identity consistency, score, and visible quality remain separate facts."}</p>{requiresEvidenceRepair && item.review ? <p className="mt-3 rounded-md bg-[var(--ad-yellow-bg)] px-3 py-2 text-xs text-[var(--ad-yellow-text)]">Earlier decision: {item.review.decision} · {item.review.identityConsistency} · {item.review.reason}</p> : null}{characterAssetReview ? <fieldset className="mt-4 grid gap-2 sm:grid-cols-2"><legend className="sr-only">Visible quality checks</legend>{reviewQualityChecks.map(([key, label]) => <label className="flex min-h-11 items-center gap-3 rounded-md border border-[var(--ad-border)] px-3 text-xs" key={key}><input checked={quality[key]} onChange={(event) => setQuality((current) => ({ ...current, [key]: event.target.checked }))} type="checkbox" /><span>{label}</span></label>)}</fieldset> : null}<div className="mt-4 grid gap-3 sm:grid-cols-[120px_180px_1fr]"><label className="text-xs font-semibold text-[var(--ad-text-muted)]">Score<input className={`${fieldClass} mt-1`} max={100} min={0} onChange={(event) => setScore(event.target.value)} placeholder="Required to approve" step={1} type="number" value={score} /></label><label className="text-xs font-semibold text-[var(--ad-text-muted)]">Identity consistency<select className={`${fieldClass} mt-1`} disabled={identityReviewMode === "defines_identity"} onChange={(event) => setIdentityConsistency(event.target.value as "passed" | "failed" | "unscored")} value={identityConsistency}><option value="passed">Passed</option><option value="failed">Failed</option><option value="unscored">{identityReviewMode === "defines_identity" ? "Unscored · defines identity" : "Unscored"}</option></select></label><label className="text-xs font-semibold text-[var(--ad-text-muted)]">Evidence and reason<textarea className={`${textAreaClass} mt-1`} onChange={(event) => setReason(event.target.value)} placeholder="Describe the visible evidence behind this decision" value={reason} /></label></div>{error ? <p className="mt-3 text-sm text-[var(--ad-red-text)]" role="alert">{error}</p> : null}{warning ? <p className="mt-3 rounded-md bg-[var(--ad-yellow-bg)] px-3 py-2 text-sm text-[var(--ad-yellow-text)]" role="status">{warning}</p> : null}<div className="mt-4 flex flex-wrap gap-2"><WorkspaceButton disabled={!permissions.review || !item.asset || busy || !validReason || !validScore || (characterAssetReview && !allQualityPassed) || (identityReviewMode === "defines_identity" && identityConsistency !== "unscored") || (identityReviewMode === "preserves_identity" && identityConsistency !== "passed")} onClick={() => void decide("approved")} tone="primary"><Check className="h-4 w-4" /> {requiresEvidenceRepair ? "Record superseding approval" : "Approve"}</WorkspaceButton><WorkspaceButton disabled={!permissions.review || !item.asset || busy || !validReason} onClick={() => void decide("rejected")} tone="danger"><X className="h-4 w-4" /> {requiresEvidenceRepair ? "Record superseding rejection" : "Reject"}</WorkspaceButton></div>{!permissions.review ? <p className="mt-3 text-xs text-[var(--ad-text-muted)]">creative.run.review is not granted.</p> : null}</section>;
}
...
function RunDetail({
  ...
  const retryCommandStatus = retryCommand ? (
    ...
    >
      <div>
        <span className="font-medium">Retry command</span>{" "}
        <StatusBadge
          value={
            retrySubmissionUnknown
              ? "outcome unknown"
