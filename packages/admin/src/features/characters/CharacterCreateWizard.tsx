...
type SaveState =
  ...
  | "Failed to save";
type ResumeState =
  "checking" | "restoring" | "restored" | "restore_failed" | "new";

class ReconciledUncommittedCharacter extends Error {}

// SPEC: 创建只问「一个角色之所以是角色」的两件事——它是谁，它长什么样。
// INTENT: 市场简报、负责人、排期和 QA 计划不影响角色成立，也不参与发布，因此不进入草稿契约。
export const characterCreateSteps = [
  "Persona",
  "Visual direction",
  "Review & create",
] as const;
...
export function isCharacterCreateStepComplete(draft: Draft, step: number) {
  if (step === 0) {
    return characterProjectDraftSchema.shape.persona.safeParse(draft.persona)
      .success;
  }
  if (step === 1) {
...
function isRecoverableLocalDraft(value: unknown): value is Draft {
  ...
  if (!isRecord(persona) || !isRecord(visualDirection)) return false;
  return (
    [
      persona.name,
...
export function CharacterCreateWizard({
    ...
    async (snapshot: Draft, allowCreate: boolean) => {
      ...
      if (
        ...
        (createIntent.status === "reconciliation_required" ||
          (recoveredCreateRequest !== null && !recoveredCreateRequest.success))
      ) {
        try {
          ...
          if (receipt.state === "committed") {
            if (
              ...
              receipt.verification.characterId !== receipt.committedTargetId
            ) {
              throw new Error(
            ...
            try {
              ...
              url.searchParams.set("draft", resumed.authority.characterId);
      ...
      try {
        if (!authorityRef.current) {
          ...
          const parsedCurrent = recoveredRequest
            ? null
            : characterProjectCreateRequestSchema.safeParse(currentRequest);
          ...
          const saved = await adminV2Operation(
        ...
        if (
          !authorityRef.current &&
          pendingCreateIntent?.status === "committed_projection_pending"
        ) {
  ...
  const persist = useCallback(
    (snapshot: Draft, allowCreate: boolean) => {
      const run = saveQueueRef.current.then(() =>
        persistNow(snapshot, allowCreate),
      );
  ...
  async function next() {
    ...
    try {
      if (authorityRef.current || createIntent) {
        ...
        setSaveState(
          saveLocalDraft(actorId, draft) ? "Saved locally" : "In memory only",
        );
      }
    ...
    >
      <header className="rounded-xl border border-[var(--ad-border)] bg-[var(--ad-surface)] p-4 sm:p-6">
        <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--ad-text-muted)]">
          {t("Character Studio ·")}{" "}
          {authority ? t("Private server draft") : t("Private draft setup")}
        </p>
        ...
        <ol
          ...
          aria-label={t("Character creation progress")}
        >
      ...
      <div className="mt-4 rounded-xl border border-[var(--ad-border)] bg-[var(--ad-surface)] p-4 sm:p-6">
        ...
        >
          {step === 0 ? (
            <PersonaStep draft={draft} errors={fieldErrors} update={update} />
          ) : null}
          {step === 1 ? (
            <VisualStep draft={draft} errors={fieldErrors} update={update} />
          ) : null}
          {step === 2 ? (
            <ReviewStep
              draft={draft}
        ...
        {resumeState === "restore_failed" ? (
          <div className="mt-4 rounded-lg border border-[var(--ad-border)] p-3">
            <p className="text-sm font-semibold">
              {t("The requested server draft was not restored.")}
            </p>
        ...
        {createIntent ? (
          <p
            className="mt-4 rounded-md bg-[var(--ad-yellow-bg)] p-3 text-sm text-[var(--ad-yellow-text)]"
            role="status"
          >
            {createIntent.status === "reconciliation_required" ||
            !characterProjectCreateRequestSchema.safeParse(
...
function SaveIndicator({ state }: { state: SaveState }) {
  ...
  return (
    <span
      className="inline-flex min-h-8 items-center gap-2 rounded-md bg-black/[0.04] px-3 text-xs font-semibold"
      role="status"
    >
      {state === "Saving" ? (
...
function Field({
  ...
  return (
    ...
    >
      ...
      {error ? (
        <span
          className="mt-1 block text-xs font-medium text-[var(--ad-red-text)]"
          id={errorId}
        >
          {t(error)}
        </span>
    ...
    <div className="space-y-4">
      <Grid>
        ...
        <Field
          ...
          label="Age (18+)"
          name="persona.age"
          onChange={(value) => set("age", Number(value))}
          type="number"
          value={draft.persona.age}
        />
        <label className="text-xs font-semibold text-[var(--ad-text-muted)]">
          ...
          <select
            className={`${fieldClass} mt-1`}
            onChange={(event) =>
              set("gender", event.target.value as Draft["persona"]["gender"])
            }
      ...
      <Grid>
        <Area
          ...
          value={draft.persona.characterPromise}
        />
        <Area
          ...
          value={draft.persona.firstMessage}
        />
      </Grid>
      <Area
        ...
        required={false}
        value={draft.persona.detailsMarkdown}
      />
...
function VisualStep({ draft, errors, update }: StepProps) {
  ...
  return (
    <Grid>
      <Area
        error={errors.identityAnchor}
        label="Identity anchor"
        name="visualDirection.identityAnchor"
        onChange={(value) => set("identityAnchor", value)}
        placeholder={t("Composed late-night radio host")}
        value={draft.visualDirection.identityAnchor}
      />
      <Area
      ...
      <label className="text-xs font-semibold text-[var(--ad-text-muted)]">
        ...
        <select
          ...
          onChange={(event) =>
            set(
              "style",
              event.target.value as Draft["visualDirection"]["style"],
            )
          }
          value={draft.visualDirection.style}
        >
      ...
      <Area
        error={errors.referenceDirection}
        label="Reference direction"
        name="visualDirection.referenceDirection"
        onChange={(value) => set("referenceDirection", value)}
        placeholder={t(
          "Low-key tungsten portraiture with an intimate editorial crop",
        )}
        value={draft.visualDirection.referenceDirection}
      />
    </Grid>
  );
}
...
function ReviewStep({
  ...
  return (
    <div className="grid gap-4">
      ...
      >
        <summary className="cursor-pointer bg-black/[0.025] px-3 py-3 text-sm font-semibold text-[var(--ad-ink)] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-[var(--ad-ink)]">
          {t("Technical details")}
        </summary>
        <div className="border-t border-[var(--ad-border)]">
          <h3 className="px-3 pt-3 text-xs font-semibold uppercase tracking-[0.12em] text-[var(--ad-text-muted)]">
