      ...
      >
        <summary className="cursor-pointer px-4 py-3">
          <span
            ...
            id="character-media-operations-title"
          >
          ...
          <DataTable
            ...
            rows={projection.operations.map((operation) => ({
              ...
              cells: [
                ...
                <div
                  className="text-xs text-[var(--ad-text-muted)]"
                  key="evidence"
                >
                  <span className="block">
                    {operation.provider?.key ?? t("Provider unavailable")}
                    {operation.attempt
                      ? ` · ${t("Attempt")} ${operation.attempt.number}`
                      : ""}
                  </span>
                  <span className="mt-1 block">
                    {t("Time")}{" "}
                    {operation.timing?.latencyMs === null ||
                    operation.timing?.latencyMs === undefined
                      ? t("Unavailable")
                    ...
                    {operation.output
                      ? t(
                          operation.output.availability === "available"
                            ? "Available"
        ...
        <p className="border-t border-[var(--ad-border)] px-4 py-2 text-xs text-[var(--ad-text-muted)]">
          {t("A completed run does not select or publish an asset.")}{" "}
          {t("Selecting an asset does not publish the Character.")}
        </p>
      </details>
