...
describe("Character unpublished change state", () => {
  it("closes release work only when live equals draft and no candidate exists", () => {
    const unchanged = characterWorkspaceDetail({
      preview: {
        live: {
          contentVersionId: "content-live",
          label: "Live",
        },
        changedFields: [],
      },
    ...
    expect(characterHasNoUnpublishedChanges(unchanged)).toBe(true);
    expect(
      characterHasNoUnpublishedChanges({
        ...unchanged,
        preview: { ...unchanged.preview, changedFields: ["description"] },
      }),
    ).toBe(false);
    expect(
      characterHasNoUnpublishedChanges({
    ...
    const ordinals = characterReleaseOrdinals([
      {
        release: {
          id: "live",
          publishedAt: "2026-07-24T00:40:28.924Z",
          createdAt: "2026-07-24T00:40:28.924Z",
        },
      },
      {
        release: {
          id: "older",
          publishedAt: "2026-07-20T04:13:28.650Z",
          createdAt: "2026-07-20T04:13:28.650Z",
        },
      },
    ]);
