      ...
      const result = await adminV2Request<{ batch: { id: string }; replayed: boolean }>(
        "/api/v2/admin/creative/runs",
        {
          ...
          body: {
            ...
            directions: selected.map((direction) => ({
              id: direction.id,
              title: direction.title,
              scenePrompt: direction.scenePrompt,
              mood: direction.mood,
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between text-xs text-[var(--ad-text-muted)]">
          ...
          <strong className="font-mono text-sm text-[var(--ad-ink)]">
            {estimatedCost === null ? t("Unavailable") : t("{cost} DC", { cost: estimatedCost })}
          </strong>
        </div>
        {pricingError ? (
          <p className="text-xs text-[var(--ad-red-text)]" role="alert">

            {t("Pricing estimate unavailable:")} {pricingError}
          </p>
        ) : null}
        ...
        <details className="rounded-md border border-[var(--ad-border)]">
          ...
          <div className="space-y-3 border-t border-[var(--ad-border)] p-3">
            ...
            <p className="text-xs text-[var(--ad-text-muted)]">
              ...
              {perItemCost === null ? t("pricing unavailable") : t("{cost} DC each", { cost: perItemCost })}
            </p>
          </div>
        </details>
      </div>
