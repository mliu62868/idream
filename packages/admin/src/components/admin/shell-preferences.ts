// SPEC: 后台外壳的两项运营偏好——界面语言、工作模式——的取值域与 cookie 编解码。
// INTENT: 这两项过去存 localStorage，只有挂载之后才读得到；于是服务端和首帧客户端必然先用
//         English + 默认工作模式，几百毫秒后再跳一次。cookie 随请求到达服务端，首帧即最终形态。
...
// INTENT: 不带 Secure —— 本地 dev 走 http，而这两项是纯展示偏好，不承载任何授权信息。
