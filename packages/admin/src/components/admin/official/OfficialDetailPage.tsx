...
function InfoGrid({ items }: { items: { label: string; value: ReactNode }[] }) {
  ...
  return (
    <dl className="grid grid-cols-2 gap-x-4 gap-y-4 sm:grid-cols-3">
      {items.map((item) => (
        <div key={item.label}>
          <dt className="text-xs text-[var(--ad-text-muted)]">{t(item.label)}</dt>
...
function TextBlock({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div>
      <p className="text-xs font-semibold text-[var(--ad-text-muted)]">{t(label)}</p>
      <p className="mt-2 whitespace-pre-wrap text-sm leading-relaxed text-[var(--ad-text)]">{value || t("Not written yet.")}</p>
    </div>
  );
}
...
export function OfficialDetailPage({ id }: { id: string }) {
  ...
  if (loading) {
    return <div className="h-72 animate-pulse rounded-lg bg-black/[0.04]" aria-label={t("Loading character workspace")} />;
  }
  ...
  const actions = mode === "edit" ? (
    ...
    <>
      ...
      {row.status === "approved" ? (
        ...
        >
          {publishReady ? t("Publish") : t("Complete release checks")}
        </PrimaryButton>
      )}
    </>
  );
    ...
    >
      ...
      {mode === "edit" && draft ? (
        ...
        <>
              ...
              <div className="p-6">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div className="max-w-2xl">
                    <p className="text-xs font-semibold uppercase tracking-wide text-[var(--ad-text-muted)]">{t("Character project")}</p>
                  ...
                  <div className="min-w-44 rounded-lg bg-black/[0.03] p-4">
                    <div className="flex items-end justify-between gap-3">
                      <span className="text-xs text-[var(--ad-text-muted)]">{t("Release readiness")}</span>
                      <strong className="text-xl tabular-nums text-[var(--ad-ink)]">{readiness?.score}%</strong>
                    </div>
                    <progress aria-label={t("{score}% release ready", { score: readiness?.score ?? 0 })} className="mt-3 h-1.5 w-full accent-[var(--ad-ink)]" max={100} value={readiness?.score ?? 0} />
                    <p className="mt-3 text-xs leading-relaxed text-[var(--ad-text-muted)]">
                      {readiness?.missing[0] ? t("Next: {item}", { item: t(readiness.missing[0]) }) : t("All release checks are complete.")}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <nav aria-label={t("Character workspace")} className="flex gap-1 overflow-x-auto border-b border-[var(--ad-border)]">
            {TABS.map((item) => {
            ...
            <div className="grid gap-6 lg:grid-cols-[1fr_0.8fr]">
              <DetailSection title={t("Profile")}>
                <InfoGrid items={[
                  ...
                  { label: t("Last updated"), value: new Date(row.updatedAt).toLocaleDateString(locale === "zh" ? "zh-CN" : "en-US") },
                ]} />
              </DetailSection>
              <DetailSection title={t("Release checklist")}>
                  ...
                  {["Core profile", "Persona", "Visual direction", "Visual identity", "Reference images", "Published artwork"].map((label) => {
                    ...
                    return (
                      <div className="flex items-center justify-between gap-3" key={label}>
                        <span className="text-sm text-[var(--ad-text)]">{t(label)}</span>
                        <span className={cn("inline-flex items-center gap-1 text-xs", complete ? "text-[var(--ad-green-text)]" : "text-[var(--ad-text-muted)]")}>
                          {complete ? <Check className="h-3.5 w-3.5" /> : null}{complete ? t("Ready") : t("Needs work")}
                        </span>
                      </div>
                    );
                  })}
          ...
          {tab === "persona" ? (
            <div className="grid gap-6 md:grid-cols-2">
              <DetailSection title={t("Character foundation")}>
              ...
              <DetailSection title={t("Conversation design")}>
                <div className="space-y-6">
          ...
          {tab === "assets" ? (
            <div className="flex flex-wrap items-center justify-between gap-4 rounded-lg border border-[var(--ad-border)] bg-[var(--ad-surface)] p-5">
              <div className="max-w-2xl">
                <p className="text-sm font-semibold text-[var(--ad-ink)]">{t("Character Asset Studio")}</p>
                <p className="mt-1 text-xs leading-5 text-[var(--ad-text-muted)]">

                  {t("Create the identity portrait, hero, and chat image pack in the dedicated workspace. Review decisions, draft selection, Release publication, and live Serving remain separate facts.")}
                </p>
              </div>
              <Link href={`/admin/characters/${row.id}?tab=assets`}>
                <PrimaryButton>{t("Open Character Asset Studio")}</PrimaryButton>
              </Link>
            </div>
          ) : null}
            ...
            <div className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
              <section className="overflow-hidden rounded-lg border border-[var(--ad-border)] bg-[var(--ad-surface)]">
                <div className="aspect-[4/5] bg-black/[0.04]">
                  {thumbnail ? (
                    // eslint-disable-next-line @next/next/no-img-element -- admin blob URLs are not compatible with next/image optimization
                    <img alt={t("{name} card preview", { name: row.name })} className="h-full w-full object-cover" src={thumbnail} />
              ...
              <DetailSection title={t("First conversation preview")}>
                ...
                <p className="mt-4 text-xs leading-relaxed text-[var(--ad-text-muted)]">{t("This preview checks the role card and opening message. Use the public preview before final release to verify the full page and chat behavior.")}</p>
              </DetailSection>
            </div>
          ) : null}
          ...
          {tab === "performance" ? (
            <DetailSection title={t("Stats")}>
              ...
              <p className="mt-5 text-xs text-[var(--ad-text-muted)]">{t("Counts are lifetime totals. Trend and conversion views can be added when time-series analytics are available.")}</p>
            </DetailSection>
          ) : null}

          {tab === "history" ? (
            <DetailSection title={t("Project history")}>
              <InfoGrid items={[
                { label: "Created", value: new Date(row.createdAt).toLocaleString() },
                { label: "Last updated", value: new Date(row.updatedAt).toLocaleString() },
                { label: "Visual identity version", value: row.visualProfile ? `v${row.visualProfile.version}` : t("Not created") },
                { label: "Current lifecycle state", value: value(row.status) },
              ]} />
              <EngineeringDetails summary={t("Engineering details")}>
                <div>{t("character.id =")} {row.id}</div>
...
function CharacterEditForm({
  ...
  return (
    <>
      ...
      <FormSection title={t("Visual direction")}>
