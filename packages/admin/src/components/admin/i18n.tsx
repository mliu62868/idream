...
const zh: Record<string, string> = {
  ...
  "This character overrides the system default. New chat speech uses the active cloned voice.": "该角色已覆盖系统默认声音，新的聊天语音将使用当前克隆声音。",
  "This character has no voice override and inherits the system voice selected for its gender.": "该角色没有专属声音覆盖，将继承按角色性别设置的系统声音。",
  ...
  "character": "角色",
  "New speech uses this system default. Existing cached clips remain unchanged.": "新的语音将使用这个系统默认声音；已缓存的历史语音不会改变。",
  ...
  "These voices apply to every character without a character-specific override. The global fallback is female by default.": "这些声音适用于所有没有角色专属覆盖的角色；全局兜底默认为女声。",
  ...
  "Read-only: generation.config.write is required to change system voice defaults.": "只读：修改系统默认声音需要 generation.config.write 权限。",
  "Female voices": "女声",
  "Male voices": "男声",
  "female voice": "女声",
  "male voice": "男声",
  ...
  "New chat speech uses this Pocket TTS voice. Existing cached clips remain unchanged.": "新的聊天语音将使用这个 Pocket TTS 声音；已缓存的历史语音不会改变。",
  "Upload one clean voice sample, verify the generated preview, and bind it to this character.": "上传一段干净的声音样本，验证生成的试听音频，再将其绑定到这个角色。",
  "clone ready": "可克隆",
  ...
  "Pocket TTS on oMLX is configured but unavailable. Verify oMLX, the pocket-tts-4bit model, and the voice adapter.": "Pocket TTS oMLX 线路已配置但当前不可用。请检查 oMLX、pocket-tts-4bit 模型和声音适配服务。",
  ...
  "Listen to the preview before changing the live character voice. Creating a candidate never changes Character.voiceId.": "请先试听，再更改线上角色声音；创建候选声音绝不会修改 Character.voiceId。",
  ...
  "Use a clean single-speaker recording. Pocket TTS on oMLX uses up to the first 30 seconds.": "请使用干净的单人录音；oMLX 上的 Pocket TTS 最多使用前 30 秒。",
  ...
  "This transcript is stored with the voice reference and sent to oMLX when synthesizing.": "转录文本会随声音参考一起保存，并在合成时发送给 oMLX。",
  "Preview script": "试听文案",
  ...
  "Scan the batch, compare two candidates when needed, then review and adopt one exact asset into the current draft slot.": "浏览整批候选图，需要时对比两张，再审核并将一张精确素材采用到当前草稿槽位。",
  ...
  "Review actions apply only to the current candidate and stay separate from draft adoption.": "审核动作只作用于当前候选图，并与草稿采用保持分离。",
  ...
  "Score identity match against the sealed Character references. Every evaluation sample requires an explicit pass or fail and a 0–100 score.": "对照已封存的角色参考图评估身份匹配。每个评测样本都必须明确选择通过或失败，并填写 0–100 分。",
  "Create the fixed 40-image identity matrix, review every image, then publish the measured route result. Character image production unlocks only after the evidence reaches the policy threshold.": "创建固定的 40 张身份评测矩阵，逐张审核后再发布实测路线结果。只有证据达到策略门槛，角色图片生产才会解锁。",
  ...
  "Batch IDs are immutable Creative Run IDs. Every sample must have a completed generation and exact automatic or human identity evidence.": "批次 ID 是不可变的创意生产批次 ID。每个样本都必须已完成生成，并具备精确的自动或人工身份一致性证据。",
  ...
  "No active reference-capable image profile can consume this Reference Set.": "当前没有可使用此参考集的启用中参考图图片配置。",
  ...
  "Identity and references are locked; the generation route is not qualified yet.": "角色身份和参考集已锁定；图片生成线路尚未通过资格验证。",
  ...
  "This preview uses the active candidate only in the current draft slot. Nothing live changes until a reviewed Release is published.": "此预览只会在当前草稿槽位使用当前候选图；只有经过审核的 Release 发布后，线上角色才会变化。",
