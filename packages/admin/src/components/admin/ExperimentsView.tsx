        ...
        <div className="grid gap-3">
          {experiments.map((row) => {
            ...
            return <article className="rounded-lg border border-[var(--ad-border)] bg-[var(--ad-surface)] p-4" key={row.id}>
              ...
              {result ? <div className="mt-4 rounded-md bg-[var(--ad-surface-muted)] p-3 text-xs" role="status"><p>Quality: {result.qualityState} · maturity: {result.maturity} · guardrails: {result.guardrailState} · significance: {result.significance}</p>{result.decisionUse === "eligible" ? <ul className="mt-2 space-y-1">{result.arms.map((arm) => <li key={arm.variant}>{arm.variant}: n={arm.matureSubjects}, rate={arm.rate === null ? "—" : `${(arm.rate * 100).toFixed(1)}%`}, lift={arm.absoluteLiftVsControl === null ? "—" : `${(arm.absoluteLiftVsControl * 100).toFixed(1)}pp`}, p={arm.pValueVsControl?.toFixed(4) ?? "—"}</li>)}</ul> : <p className="mt-2 text-[var(--ad-yellow-text)]">Lift hidden from decision use until every arm has ≥{result.minimumMaturePerArm} mature production exposures and guardrails pass.</p>}</div> : null}
            </article>;
          })}
        </div>
      </section>
