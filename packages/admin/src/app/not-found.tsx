...
//       以及最接近的几个真实目的地。
// INTENT: 这一页原来住在 app/admin/not-found.tsx。src/proxy.ts 把认不出的 /admin/* rewrite 到
//         Next 的 /_not-found（那是唯一能在流式输出开始前把状态码定成 404 的位置），而
//         /_not-found 渲染的是根 not-found —— 所以页面搬到了 app/ 根下，admin 段那一份已删。
// INTENT: 和 error.tsx、ui/Toast.tsx 一样，这一页在 AdminI18nProvider 之外，直接查同一张
//         词典，不是第二套 i18n。
// INVARIANT: 建议只按 URL 猜，不按权限筛 —— 这一页在 bootstrap 之外渲染，拿不到 actor 的
//            权限集。猜中一个运营打不开的工作台，他至少会撞上「缺哪几个键、找谁授予」那一页；
//            一条建议都不给才是真正的死胡同。
