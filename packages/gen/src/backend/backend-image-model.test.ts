...
it("allows a measured 332-second local identity run beyond the legacy pipeline budget", async () => {
  vi.stubEnv("PIPELINE_TIMEOUT_MS", "300000");
  vi.stubEnv("GEN_IMAGE_TIMEOUT_MS", undefined);
  let executionBudget = 0;
  const backend = makeStubBackend({
    submit: vi.fn(async (job) => {
      executionBudget = job.timeoutMs;
      return { id: "existing-identity-prompt" };
    }),
    poll: vi.fn(async () => {
      if (executionBudget < 332_000) throw new Error("execution timed out before provider success");
      return { assets: [{ body: PNG, width: 832, height: 1024, contentType: "image/png" }] };
    }),
  });
  const result = await modelWith(backend).generate({ prompt: "Adult portrait", model: "m", count: 1, controls: PIN });
  expect(result.ok).toBe(true);
  expect(backend.submit).toHaveBeenCalledTimes(1);
});

it("passes the explicit workflow image execution budget to the backend", async () => {
  vi.stubEnv("GEN_IMAGE_TIMEOUT_MS", "420000");
  const backend = makeStubBackend();
  await modelWith(backend).generate({ prompt: "Adult portrait", model: "m", count: 1, controls: PIN });
  expect(backend.submit).toHaveBeenCalledWith(expect.objectContaining({ timeoutMs: 420_000 }));
});
