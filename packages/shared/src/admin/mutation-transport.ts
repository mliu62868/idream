...
export const ADMIN_V2_MUTATION_TRANSPORT = {
  ...
  "POST /api/v2/admin/cases/:id/actions": pending("case-platform", "Case action writes are not protected from duplicate submission."),
  "POST /api/v2/admin/cases/:id/assignment": pending("case-platform", "Assignment retries can repeat timeline and audit side effects."),
  "POST /api/v2/admin/cases/:id/commands/close": idempotencyKey(),
  "POST /api/v2/admin/cases/:id/commands/reopen": pending("case-platform", "Reopen retries are not bound to one stable command result."),
  "POST /api/v2/admin/cases/:id/commands/wait": pending("case-platform", "Wait-state retries can append duplicate operational events."),
  "POST /api/v2/admin/cases/:id/decisions": pending("case-platform", "Decision retries can duplicate authoritative decision side effects."),
  "POST /api/v2/admin/cases/:id/verification": pending("case-platform", "Verification retries can duplicate evidence and audit entries."),
  ...
  "PATCH /api/v2/admin/incidents/:id": pending("incident-platform", "Triage updates have no optimistic concurrency precondition.", "If-Match"),
  "POST /api/v2/admin/incidents/:id/action-plans/preview": pending("incident-platform", "Preview creation persists a plan and can duplicate it on retry."),
  "POST /api/v2/admin/incidents/:id/action-plans/:planId/execute": idempotencyKey(),
  "POST /api/v2/admin/incidents/:id/commands/close": pending("incident-platform", "Close retries are not bound to one stable command result."),
  "POST /api/v2/admin/incidents/:id/commands/merge": pending("incident-platform", "Merge retries can repeat cross-incident mutations."),
  "POST /api/v2/admin/incidents/:id/commands/resolve": idempotencyKey(),
  "POST /api/v2/admin/incidents/:id/commands/split": pending("incident-platform", "Split retries can create duplicate child incidents."),
  "POST /api/v2/admin/incidents/:id/verification": pending("incident-platform", "Recovery verification retries can duplicate evidence and events."),
