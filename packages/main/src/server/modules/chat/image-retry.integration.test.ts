...
describe("Chat image retry delivery authority", () => {
  it("rebinds once before dispatch, delivers in the same Chat, and reuses that delivery on regenerate", async () => {
    ...
    const supersededQuote = await api("POST", `generation/jobs/${original.id}/retry/quote`, { userId: f.userId, ageGate: true });
    expectError(supersededQuote, 409);
    expect(supersededQuote.json).toMatchObject({ error: { message: "This Chat image is no longer available to retry. Check the chat for its current result." } });
    ...
    expectError(await api("POST", `generation/jobs/${original.id}/retry/quote`, { userId: f.userId, ageGate: true }), 409);
    ...
    const replay = await f.retry(`${f.userId}:retry`);
    expectOk(replay, 202);
    expect(replay.data.job.id).toBe(jobId);
