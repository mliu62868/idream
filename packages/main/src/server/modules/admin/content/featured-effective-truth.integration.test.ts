...
describe("Featured configured and effective runtime truth", () => {
  ...
  it("uses version zero for a missing setting and creates version one", async () => {
    ...
    const missingVersion = await api("PUT", "admin/content/featured", {
      userId: adminId,
      role: "admin",
      headers: {
    ...
    const response = await api("PUT", "admin/content/featured", {
      userId: adminId,
      role: "admin",
      headers: {
  ...
  it("allows exactly one of two operators to save the same setting version", async () => {
    ...
    const attempts = await Promise.all(
      targets.map((characterId, index) =>
        api("PUT", "admin/content/featured", {
...
async function featured() {
  const response = await api("GET", "admin/content/featured", {
    userId: adminId,
    role: "admin",
  });
  expectOk(response);
