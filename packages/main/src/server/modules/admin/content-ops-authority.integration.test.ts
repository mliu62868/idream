...
describe("Image Library and legacy Placement authority", () => {
  ...
  function request(
    ...
    const placementPatch = method === "PATCH" &&
      path.startsWith("admin/content/placements/");
  ...
  it("projects live Character Release dependencies and blocks Library review or archive", async () => {
    const response = await getContentAsset(
      request("GET", `admin/content/assets/${releaseAssetId}`),
      releaseAssetId,
    );
    ...
    await expect(patchContentAsset(
      request("PATCH", `admin/content/assets/${releaseAssetId}`, {
        status: "archived",
  ...
  it("blocks Library archive for a direct Character primary image without other authority records", async () => {
    const response = await getContentAsset(
      request("GET", `admin/content/assets/${primaryImageAssetId}`),
      primaryImageAssetId,
    );
    await expect(response.json()).resolves.toMatchObject({
      data: {
    ...
    await expect(patchContentAsset(
      request("PATCH", `admin/content/assets/${primaryImageAssetId}`, {
        status: "archived",
  ...
  it("keeps Library metadata separate from Creative purpose, review evidence, and tags", async () => {
    ...
    const rejected = await api(
      "PATCH",
      `admin/content/assets/${freeAssetId}`,
      {
        userId: actorId,
        role: "admin",
        body: {
    ...
    const descriptionOnly = await api(
      "PATCH",
      `admin/content/assets/${freeAssetId}`,
      {
        userId: actorId,
  ...
  it("projects scheduled Character Release dependencies and blocks single or bulk archive", async () => {
    ...
    const response = await getContentAsset(
      request("GET", `admin/content/assets/${scheduledReleaseAssetId}`),
      scheduledReleaseAssetId,
    );
    const payload = await response.json();
    expect(payload.data.asset.authorityDependencies).toEqual(
    ...
    await expect(patchContentAsset(
      request("PATCH", `admin/content/assets/${scheduledReleaseAssetId}`, {
        status: "archived",
    ...
    await expect(bulkPatchContentAssets(
      request("PATCH", "admin/content/assets/bulk", {
  ...
  it("ignores deleted Character image authorities and retired or inactive Project drafts", async () => {
    ...
    try {
      const response = await getContentAsset(
        request("GET", `admin/content/assets/${staleAssetId}`),
        staleAssetId,
      );
      ...
      await expect(
        patchContentAsset(
          request("PATCH", `admin/content/assets/${staleAssetId}`, {
            status: "archived",
  ...
  it("projects verified Campaign dependencies and makes v2 placements read-only to legacy PATCH", async () => {
    const response = await getContentAsset(
      request("GET", `admin/content/assets/${campaignAssetId}`),
      campaignAssetId,
    );
    const payload = await response.json();
    ...
    await expect(patchContentAsset(
      request("PATCH", `admin/content/assets/${campaignAssetId}`, {
        status: "archived",
    ...
    for (const patch of attempts) {
      await expect(patchPlacement(
        request("PATCH", `admin/content/placements/${placementId}`, {
          ...patch,
  ...
  it("treats published Run items as history after their Campaign placement is replaced", async () => {
    ...
    try {
      const oldDetail = await getContentAsset(
        request("GET", `admin/content/assets/${oldAssetId}`),
        oldAssetId,
      );
      ...
      await expect(
        patchContentAsset(
          request("PATCH", `admin/content/assets/${oldAssetId}`, {
            status: "archived",
      ...
      const currentDetail = await getContentAsset(
        request("GET", `admin/content/assets/${currentAssetId}`),
        currentAssetId,
      );
      ...
      await expect(
        patchContentAsset(
          request("PATCH", `admin/content/assets/${currentAssetId}`, {
            status: "archived",
  ...
  it("dispatches the documented POST bulk route and the single batch preflight route", async () => {
    ...
    try {
      const preflight = await api(
        "POST",
        "admin/content/assets/bulk/preflight",
        {
          userId: actorId,
          role: "admin",
      ...
      const archived = await api("POST", "admin/content/assets/bulk", {
        userId: actorId,
        role: "admin",
  ...
  it("rejects unsorted or duplicate bulk mutation targets before any lookup or lock", async () => {
    ...
    const unsorted = await api("POST", "admin/content/assets/bulk", {
      userId: actorId,
      role: "admin",
      body: {
    ...
    const duplicate = await api("POST", "admin/content/assets/bulk", {
      userId: actorId,
      role: "admin",
      body: {
  ...
  it("re-reads every target after authority locks and rejects an asset deleted while waiting", async () => {
    ...
    let pendingArchive: Promise<Awaited<ReturnType<typeof api>>> | null = null;
    try {
      await prisma.$transaction(async (tx) => {
        await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtext(${`media-asset-authority:${raceAssetId}`}))`;
        pendingArchive = api("POST", "admin/content/assets/bulk", {
          userId: actorId,
          role: "admin",
  ...
  it("archives lifecycle state without replaying stale tags or description", async () => {
    ...
    try {
      const response = await patchContentAsset(
        request("PATCH", `admin/content/assets/${metadataAssetId}`, {
          status: "archived",
          tags: ["stale-tag"],
  ...
  it("blocks archive while a queued generation job pins an image only through controls", async () => {
    ...
    try {
      const detail = await getContentAsset(
        request("GET", `admin/content/assets/${freeAssetId}`),
        freeAssetId,
      );
      ...
      await expect(
        patchContentAsset(
          request("PATCH", `admin/content/assets/${freeAssetId}`, {
            status: "archived",
  ...
  it("protects a Look-only generation input from single and bulk archive until the job is terminal", async () => {
    ...
    try {
      const detail = await getContentAsset(
        request("GET", `admin/content/assets/${lookAssetId}`),
        lookAssetId,
      );
      expect(detail.status).toBe(200);
      expect(await detail.json()).toMatchObject({
      ...
      await expect(
        patchContentAsset(
          request("PATCH", `admin/content/assets/${lookAssetId}`, {
            status: "archived",
  ...
  it("ignores stale active Reference Sets from archived Visual Profiles", async () => {
    ...
    try {
      const oldOnlyDetail = await getContentAsset(
        request("GET", `admin/content/assets/${oldOnlyAssetId}`),
        oldOnlyAssetId,
      );
      ...
      await expect(
        patchContentAsset(
          request("PATCH", `admin/content/assets/${oldOnlyAssetId}`, {
            status: "archived",
  ...
  it("serializes Library archive against placement staging", async () => {
    ...
    const results = await Promise.allSettled([
      patchContentAsset(
        request("PATCH", `admin/content/assets/${approvedAssetId}`, {
          status: "archived",
  ...
  it("replays standalone draft Placement creation by idempotency key", async () => {
    ...
    const createRequest = () => new Request(
      "http://localhost/admin/content/placements",
      {
        method: "POST",
        headers: {
          "content-type": "application/json",
  ...
  it("requires Placement transport headers and replays PATCH after response loss", async () => {
    ...
    const created = await api("POST", "admin/content/placements", {
      userId: actorId,
      role: "admin",
      body: {
        mediaAssetId: concurrentAssetId,
    ...
    const patchRequest = (ifMatch: string | null, idempotencyKey = patchKey) =>
      new Request(`http://localhost/admin/content/placements/${placementId}`, {
        method: "PATCH",
        headers: {
          "content-type": "application/json",
          "x-idream-user-id": actorId,
    ...
    await expect(patchPlacement(
      ...
      status: 400,
      message: "If-Match must contain the current Placement version",
    });
  ...
  it("keeps standalone legacy Placement PATCH actions explicit and archive terminal", async () => {
    ...
    for (const status of ["draft", "scheduled", "published"] as const) {
      const rejected = await api(
        "PATCH",
        `admin/content/placements/${legacyPlacementId}`,
        {
          userId: actorId,
          role: "admin",
          body: {
  ...
  it("serializes Library archive against placement verification", async () => {
    const results = await Promise.allSettled([
      patchContentAsset(
        request("PATCH", `admin/content/assets/${concurrentAssetId}`, {
          status: "archived",
  ...
  it.each(["scheduled", "published", "paused", "archived"] as const)(
    ...
    async (status) => {
      ...
      const result = await api("POST", "admin/content/placements", {
        userId: actorId,
        role: "admin",
        body: {
          mediaAssetId: approvedAssetId,
