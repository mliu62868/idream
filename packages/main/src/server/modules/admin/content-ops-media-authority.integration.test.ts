...
describe("legacy content operations media authority", () => {
  ...
  it("retains demo assets in private admin projections and marks them synthetic", async () => {
    ...
    for (const fixture of fixtures) {
      const batch = await api(
        "GET",
        `admin/content/production/batches/${fixture.batchId}`,
        { userId: adminId, role: "admin" },
      );
      expectOk(batch);
      expect(batch.data.batch.items[0].asset).toMatchObject({
        id: fixture.assetId,
        ...fixture.expected,
      });
    }
    ...
    const asset = await api(
      "GET",
      `admin/content/assets/${fixtures[1].assetId}`,
      { userId: adminId, role: "admin" },
    );
    expectOk(asset);
    expect(asset.data.asset).toMatchObject({
      id: fixtures[1].assetId,
      ...fixtures[1].expected,
    });
    await expect(
    ...
    const archivedPlacement = await api("POST", "admin/content/placements", {
      userId: adminId,
      role: "admin",
      body: {
  ...
  it("rejects a canonically approved mock asset at the remaining draft Placement boundary", async () => {
    ...
    const response = await api("POST", "admin/content/placements", {
      userId: adminId,
      role: "admin",
      body: {
