...
const createVisualProfileSchema = z.object({
  identityPrompt: z.string().trim().min(1).max(2_000),
...
// adapterRefs 有意排除：面板不展示 LoRA/adapter 之类的生成模型接线细节。
...
export async function createCharacterVisualProfile(
  ...
  const created = await prisma.$transaction(async (tx) => {
    ...
    return tx.characterVisualProfile.create({
      data: {
        ...
        version,
        status: "active",
        style: body.style ?? active?.style ?? "realistic",
        identityPrompt: body.identityPrompt,
        negativeIdentityPrompt: body.negativeIdentityPrompt ?? active?.negativeIdentityPrompt ?? null,
        faceTraits: toInputJson(body.faceTraits ?? active?.faceTraits ?? {}),
        hairTraits: toInputJson(body.hairTraits ?? active?.hairTraits ?? {}),
        bodyTraits: toInputJson(body.bodyTraits ?? active?.bodyTraits ?? {}),
        signatureTraits: toInputJson(body.signatureTraits ?? active?.signatureTraits ?? {}),
        styleTraits: toInputJson(body.styleTraits ?? active?.styleTraits ?? {}),
  ...
  await writeAudit(request, actor, {
    ...
    reason: body.reason,
    after: { visualProfileId: created.id, version: created.version, status: created.status },
  });
