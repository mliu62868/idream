...
const chatMemoryEditSchema = z.object({
...
const relationshipEditSchema = z.object({
  ...
  boundaries: z.array(z.string().trim().min(1).max(200)).max(50).optional(),
});
...
async function dispatchV1Unsafe(request: Request, segments: string[]) {
  ...
  if (resource === "chat") {
    ...
    if (id === "memories" && !action && method === "GET") return listChatMemories(request);
    if (id === "memories" && action && !child && method === "PATCH") {
      return updateChatMemory(request, action);
    }
    if (id === "memories" && action && !child && method === "DELETE") {
      return deleteChatMemory(request, action);
    }
    if (id === "relationships" && !action && method === "GET") {
      return listRelationships(request);
    }
    if (id === "relationships" && action && !child && method === "GET") {
      return getRelationship(request, action);
    }
    if (id === "relationships" && action && !child && method === "PATCH") {
      return updateRelationship(request, action);
    }
    if (id === "relationships" && action && !child && method === "DELETE") {
      return deleteRelationship(request, action);
    }
    if (id === "streams" && action && method === "GET") return streamChatMessage(request, action);
    ...
    if (id === "sessions" && action && child === "no-memory" && method === "POST") {
      return setChatSessionNoMemory(request, action);
    }
...
async function listChatMemories(request: Request) {
  const ctx = await getAuthCtx(request);
  const user = requireUser(ctx);
  const url = new URL(request.url);
  ...
  const items = await prisma.companionMemory.findMany({
    where: {
      userId: user.id,
      status: "active",
      deletedAt: null,
...
async function updateChatMemory(request: Request, id: string) {
  const ctx = await getAuthCtx(request);
  const user = requireUser(ctx);
  const body = chatMemoryEditSchema.parse(await jsonBody(request));
  const memory = await prisma.companionMemory.findFirst({
    where: { id, userId: user.id },
  });
  if (!memory) throw Errors.notFound("Memory not found");

  const deleted = body.status === "deleted";
  const updated = await prisma.companionMemory.update({
    where: { id: memory.id },
    data: {
      text: body.text ?? memory.text,
      status: body.status ?? memory.status,
      deletedAt: deleted ? new Date() : memory.deletedAt,
    },
  });

  if (deleted) {
    await enqueueMemoryForget({
      userId: user.id,
      memoryIds: [memory.id],
...
async function deleteChatMemory(request: Request, id: string) {
  const ctx = await getAuthCtx(request);
  const user = requireUser(ctx);
  const memory = await prisma.companionMemory.findFirst({
    where: { id, userId: user.id },
  ...
  await prisma.companionMemory.update({
    where: { id: memory.id },
...
async function listRelationships(request: Request) {
  const ctx = await getAuthCtx(request);
  const user = requireUser(ctx);
  const url = new URL(request.url);
  const characterId = url.searchParams.get("characterId") ?? undefined;
  const items = await prisma.relationshipState.findMany({
...
async function getRelationship(request: Request, characterId: string) {
  const ctx = await getAuthCtx(request);
  const user = requireUser(ctx);
  ...
  if (!relationship) throw Errors.notFound("Relationship not found");
  return ok({ relationship });
}

async function updateRelationship(request: Request, characterId: string) {
  const ctx = await getAuthCtx(request);
  const user = requireUser(ctx);
  await readableCharacter(characterId, user.id);
  const body = relationshipEditSchema.parse(await jsonBody(request));
  const existing = await prisma.relationshipState.findUnique({
    where: { userId_characterId: { userId: user.id, characterId } },
  });
  const relationship = await prisma.relationshipState.upsert({
    where: { userId_characterId: { userId: user.id, characterId } },
    update: {
      stage: body.stage,
      summary: body.summary === undefined ? undefined : body.summary,
      boundaries: body.boundaries ? toInputJson(body.boundaries) : undefined,
      version: { increment: 1 },
    },
    create: {
      userId: user.id,
      ...
      signals: toInputJson({}),
      boundaries: toInputJson(body.boundaries ?? []),
...
async function deleteRelationship(request: Request, characterId: string) {
  const ctx = await getAuthCtx(request);
  const user = requireUser(ctx);
  ...
  await enqueueMemoryForget({
    ...
    reason: "runtime_rebuild",
  });
  return ok({ deleted: true });
}

async function streamChatMessage(request: Request, assistantMessageId: string) {
  const ctx = await getAuthCtx(request);
  const user = requireUser(ctx);
  const message = await prisma.message.findFirst({
    where: {
      ...
      role: "assistant",
      session: { userId: user.id, status: { not: "deleted" } },
    },
  });
  if (!message) throw Errors.notFound("Message stream not found");
  return createChatSseResponse(
    `chat:stream:${assistantMessageId}`,
...
async function sendChatMessage(request: Request, sessionId: string) {
  ...
  const session = await prisma.chatSession.findFirst({
    ...
    include: {
      character: { include: { tags: { include: { tag: true } } } },
  ...
  const streamKey = `chat:stream:${assistant.id}`;
  const entitlements = await entitlementMap(user.id);
  const entitlementSnapshot = {
    modelTier: modelTier(entitlements),
    memoryMultiplier: memoryMultiplier(entitlements),
    unlimitedMessages: booleanEntitlement(entitlements.unlimited_messages),
  };
  const preferences = await ensurePreferences(user.id);
  const memoryEnabled = session.memoryEnabled;
  const streamUrl = `/api/v1/chat/streams/${assistant.id}`;
  const history = [
    ...session.messages
  ...
  const [userProfile, longTermMemories, relationshipState] = await Promise.all([
      ...
      ? prisma.companionMemory.findMany({
          where: {
            ...
            OR: [
              ...
              { scope: "global", characterId: null },
              { sessionId: session.id },
            ],
          },
          orderBy: [{ confidence: "desc" }, { updatedAt: "desc" }],
          take: Math.max(6, 12 * entitlementSnapshot.memoryMultiplier),
        })
      : Promise.resolve([]),
    prisma.relationshipState.findUnique({
      where: { userId_characterId: { userId: user.id, characterId: session.characterId } },
    }),
  ]);
  const payload: ChatGeneratePayload = {
    version: 1,
    kind: "chat.generate",
    requestId: cryptoRandomId("chat_req"),
    sessionId,
    userMessageId: userMessage.id,
    assistantMessageId: assistant.id,
    streamKey,
    mode: memoryEnabled ? "normal" : "no_memory",
    ...
    user: {
      ...
      memoryEnabled,
      mutedTags: jsonStringArray(preferences.mutedTags),
      safeModeFlags: jsonRecord(preferences.safeModeFlags),
    },
    character: {
      characterId: session.character.id,
      name: session.character.name,
      age: session.character.age,
      relationship: session.character.relationship,
      description: session.character.description,
      systemPrompt: session.character.systemPrompt,
      tags: session.character.tags.map(({ tag }) => tag.slug),
    },
    context: {
      sessionSummary: session.memorySummary,
      ...
      longTermMemories: memoryEnabled
      ...
      relationshipState: relationshipState
        ? {
            ...
            summary: relationshipState.summary,
            signals: relationshipState.signals,
            boundaries: relationshipState.boundaries,
            version: relationshipState.version,
          }
        : null,
    },
    policy: {
  ...
  await jobQueue.enqueue({
    queue: "ai.chat.generate",
    payload: toInputJson(payload),
    dedupeKey: `chat:${assistant.id}`,
  });
...
async function setChatSessionNoMemory(request: Request, id: string) {
  const ctx = await getAuthCtx(request);
  const user = requireUser(ctx);
  const session = await prisma.chatSession.findFirst({
    where: { id, userId: user.id, status: { not: "deleted" } },
  ...
  const updated = await prisma.chatSession.update({
    where: { id: session.id },
    data: { memoryEnabled: false, memorySummary: null },
  });

  await enqueueMemoryForget({
    userId: user.id,
    sessionId: session.id,
    reason: "session_no_memory",
  });

  await trackEvent("chat_no_memory_enabled", { sessionId: session.id }, ctx);
  return ok({ session: updated, memoryEnabled: false });
}
...
async function deleteMessage(request: Request, id: string) {
  ...
  const memoryIds = await softDeleteMemoriesBySourceMessageId(user.id, message.id);
  await enqueueMemoryForget({
    userId: user.id,
    memoryIds,
  ...
  return ok({ deleted: true, forgottenMemoryIds: memoryIds });
}
...
function booleanEntitlement(value: Prisma.JsonValue | undefined) {
  return value === true;
}

function modelTier(entitlements: Record<string, Prisma.JsonValue>) {
  const plan = jsonRecord(entitlements.plan);
  const slug = typeof plan.slug === "string" ? plan.slug : "";
  if (slug.includes("deluxe") || booleanEntitlement(entitlements.video_generation)) {
    return "deluxe" as const;
  }
  if (slug.includes("premium") || booleanEntitlement(entitlements.premium_controls)) {
    return "premium" as const;
  }
  return "free" as const;
}

function memoryMultiplier(entitlements: Record<string, Prisma.JsonValue>) {
  return modelTier(entitlements) === "deluxe" ? 3 : 1;
}
...
async function enqueueMemoryForget(input: {
  userId: string;
  scope?: MemoryForgetPayload["scope"];
  targetIds?: string[];
  sessionId?: string;
  memoryIds?: string[];
  sourceMessageId?: string;
  reason: MemoryForgetPayload["reason"];
}) {
  const target = [
    ...(input.targetIds ?? []),
    input.sessionId,
    ...
    ...(input.memoryIds ?? []),
  ].filter(Boolean).join(":") || cryptoRandomId("memory_forget");
  const payload: MemoryForgetPayload = {
    version: 1,
    kind: "memory.forget",
    requestId: cryptoRandomId("memory_req"),
    userId: input.userId,
    scope: input.scope ?? (input.sourceMessageId ? "message" : input.memoryIds?.length ? "memory" : undefined),
    targetIds: input.targetIds ?? (input.sourceMessageId ? [input.sourceMessageId] : input.memoryIds ?? []),
  ...
  await jobQueue.enqueue({
    queue: "ai.memory.forget",
    payload: toInputJson(payload),
    dedupeKey: `memory-forget:${input.reason}:${input.userId}:${target}`,
  });
  await drainLocalAiPipeline({ queues: [...localAiQueueNames], limit: 8 });
}

async function softDeleteMemoriesBySourceMessageId(userId: string, sourceMessageId: string) {
  const memories = await prisma.companionMemory.findMany({
    where: { userId, status: "active", deletedAt: null },
    select: { id: true, sourceMessageIds: true },
  });
  const ids = memories
    .filter((memory) => jsonStringArray(memory.sourceMessageIds).includes(sourceMessageId))
