...
async function dispatchV1Unsafe(request: Request, segments: string[]) {
  ...
  // Main owns Chat product state and idempotency. proxyChatRequest is a legacy
  // name for the Turn Ledger façade; only AgentRun admission/cancel/SSE crosses
  // the Chat service boundary, which remains a hard execution dependency.
  ...
  if (customerCareResponse) return customerCareResponse;
...
async function openEditDraft(request: Request, characterId: string) {
  ...
  requireAgeGate(ctx);
  requireAgeVerified(ctx);
  const { draft, character } = await openCharacterEditDraft({ userId: user.id, characterId });
...
async function submitDraft(request: Request, id: string) {
  ...
  const { character, edited } = await submitCharacterDraft({
    userId: user.id,
    draftId: id,
