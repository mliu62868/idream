...
// SPEC: Discovery and direct links share exactly the same release, creator,
// image and age authority. Visibility only decides whether discovery lists it.
const releasedCharacterAudienceWhere = {
  AND: [
    { age: { gte: 18 } },
    {
      OR: [
...
export const publicCharacterAudienceWhere = {
  AND: [rawPublicCharacterWhere, releasedCharacterAudienceWhere],
} as const satisfies Prisma.CharacterWhereInput;

// Create promises that unlisted Characters remain reachable by direct link.
export const directCharacterAudienceWhere = {
  AND: [
    { visibility: { in: ["public", "unlisted"] }, status: "approved", deletedAt: null },
    releasedCharacterAudienceWhere,
  ],
} as const satisfies Prisma.CharacterWhereInput;
