...
export async function getCharacterWorkspace(characterId: string) {
  ...
  const servingContentVersionId =
    serving?.currentRelease?.characterContentVersionId ?? null;
  const servingContentVersion = servingContentVersionId
    ...
    : null;
  const soul = characterSoulWorkspaceProjection(
    contentVersions,
  ...
  const [characterImageReferenceCount, mediaOperations] = await Promise.all([
    character.imageAsset?.characterId === null
      ? prisma.character.count({
          where: { imageAssetId: character.imageAsset.id },
        })
      : Promise.resolve(0),
  ...
  const latestValidationIds = Array.from(
    new Set(
      releaseIds.flatMap((releaseId) => {
        const latest = validationRuns.find(
          (run) => run.releaseId === releaseId,
        );
  ...
  const previewAssetIds = [
    ...new Set([
      ...
      ...releases.flatMap((release) =>
        Object.values(releasePreviewAssetPackIds(release)),
      ),
    ]),
  ];
  const [checks, monitors, contents, releaseImageAssets] = await Promise.all([
    prisma.releaseCheckResult.findMany({
      where: { validationRunId: { in: latestValidationIds } },
    }),
    prisma.releaseMonitor.findMany({
      where: { releaseId: { in: releaseIds } },
      orderBy: { startedAt: "desc" },
    }),
    prisma.characterContentVersion.findMany({
  ...
  const characterImageAvailable =
    ...
    (character.imageAsset.characterId === characterId ||
      (character.imageAsset.characterId === null &&
        characterImageReferenceCount === 1));
  ...
  const previewAssetById = new Map(
    releaseImageAssets.map((asset) => [asset.id, asset]),
  );
  const currentRelease =
    serving?.state === "live"
      ? (releases.find((release) => release.id === serving.currentReleaseId) ??
        null)
      : null;
  ...
  const liveContent =
    contents.find(
      (content) => content.id === currentRelease?.characterContentVersionId,
    ) ?? null;
  const draftContent =
    contents.find(
      (content) => content.id === candidateRelease?.characterContentVersionId,
    ) ??
    contents[0] ??
    null;
  const live = currentRelease
    ? previewSnapshot({
        character,
        content: liveContent,
  ...
  const portfolio = await listCharacterPortfolioData(
    prisma,
    {
      limit: 1,
      search: characterId,
  ...
  const portfolioItem =
    portfolio.items.find((item) => item.characterId === characterId) ?? null;
  if (!portfolioItem) {
    throw new Error(`Character production journey missing for ${characterId}`);
  }
  const performance = portfolioItem.performance;
  return {
    character: {
      id: character.id,
      name: character.name,
    ...
    activeCommand: activeCommand
      ? adminCommandStatusSchema.parse({
          commandId: activeCommand.id,
          requestId: activeCommand.requestId,
          commandType: activeCommand.commandType,
          target: {
            type: activeCommand.targetType,
            id: activeCommand.targetId,
          },
          status: activeCommand.status,
          verificationState:
            activeCommand.status === "verifying" ? "verifying" : "pending",
