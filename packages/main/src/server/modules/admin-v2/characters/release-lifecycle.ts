...
export async function createCharacterRelease(
  input: {
    request: Request;
    characterId: string;
  ...
  const actor =
    input.actor ??
    (await actorWithPermission(input.request, "character.release.publish", {
  ...
  const execute = async (tx: Prisma.TransactionClient) => {
    ...
    await lockCharacterMediaAssetAuthorities(tx, authorityLockedAssetIds);
    ...
    if (existing)
      throw Errors.conflict(
        "Character already has a Release ready to publish",
    ...
    const exactDraftAssetPackAuthority =
      profile && referenceSet && route
        ...
        : null;
    const coverPackEntry = draftPackEntry(
      project.draftAssetPack,
      "character_cover",
    );
    const draftAssetEntries = [
      {
        ...
        (!project.draftImageAssetId ||
          coverPackEntry.assetId === project.draftImageAssetId)
          ? coverPackEntry
          : {
              assetId:
                project.draftImageAssetId ?? character?.imageAssetId ?? null,
              runId: null,
              itemId: null,
              reviewDecisionId: null,
              generationJobId: null,
      ...
      },
      {
        ...
        ...draftPackEntry(project.draftAssetPack, "character_hero"),
      },
      {
        purpose: "character_chat",
        slotKey: "character_chat",
        ...draftPackEntry(project.draftAssetPack, "character_chat"),
      },
    ].filter(
      (entry): entry is { purpose: string; slotKey: string } & DraftPackEntry =>
        typeof entry.assetId === "string",
    );
    const selectedEntries = draftAssetEntries.filter(
      (entry) => entry.selectedFromProject,
    );
    ...
    // 这个问题不再存在。所以它留在发布创建侧，不并进规则引擎。
    const blockers = [
      ...characterReleaseBlockers(evaluation.failed),
    ...
    if (blockers.length > 0)
      throw Errors.conflict("Character is not ready to publish", { blockers });
    ...
    if (validation.failed.length > 0) {
      throw Errors.conflict("Character is not ready to publish", {
        blockers: validation.failed.map((check) => check.key),
      });
    }
    const readyRelease = await tx.characterRelease.update({
      where: { id: release.id },
    ...
    await tx.adminAuditLog.create({
      data: {
        ...
        action: "character.release.prepared",
        ...
        after: toInputJson({
          ...
          snapshotHash: readyRelease.snapshotHash,
          status: readyRelease.status,
    ...
    await tx.mainOutboxEvent.create({
      data: {
        eventType: "character.release.prepared.v2",
        aggregateType: "character_release",
        aggregateId: readyRelease.id,
        payload: toInputJson({
          characterId: input.characterId,
          releaseId: readyRelease.id,
          snapshotHash: readyRelease.snapshotHash,
          version: readyRelease.version,
          validationRunId: validation.run.id,
        }),
      },
    });
