...
describe("Character Reference Set publication", () => {
  ...
  it("removes a deselected reference from runtime authority and the next identity version", async () => {
    ...
    const archived = await patchContentAsset(
      new Request(
        `http://localhost/api/v2/admin/assets/${assetIds[1]}`,
        {
          method: "PATCH",
          headers: {
            "idempotency-key": `${prefix}library-1`,
            "content-type": "application/json",
            "x-idream-user-id": actorId,
            "x-idream-role": "admin",
          },
  ...
  it("never seals an archived current image or inherited anchor into a Visual Profile", async () => {
    ...
    await expect(patchContentAsset(
      new Request(
        `http://localhost/api/v2/admin/assets/${fallbackAssetId}`,
        {
          method: "PATCH",
          headers: {
            "idempotency-key": `${prefix}library-2`,
            "content-type": "application/json",
            "x-idream-user-id": actorId,
            "x-idream-role": "admin",
          },
    ...
    await expect(
      createCharacterVisualProfile(
        new Request(
          `http://localhost/api/v2/admin/content/characters/${fallbackCharacterId}/visual-profiles`,
          {
            method: "POST",
            headers: {
              "content-type": "application/json",
    ...
    await expect(
      createCharacterVisualProfile(
        new Request(
          `http://localhost/api/v2/admin/content/characters/${inheritedCharacterId}/visual-profiles`,
          {
            method: "POST",
            headers: {
              "content-type": "application/json",
