...
function defineTransitionAuthority<const States extends readonly string[]>(
  ...
  return {
    permits(from: string, to: string) {
      ...
      return transitions[from as StateOf<States>].some(
...
// Release creation either prepares an approved immutable candidate or imports
// an already-published immutable snapshot. There is no operator review state.
...
const CHARACTER_SERVING_AUTHORITY = defineTransitionAuthority(
...
const GENERATION_REQUEST_AUTHORITY = defineTransitionAuthority(
...
const GENERATION_ATTEMPT_AUTHORITY = defineTransitionAuthority(
...
export const CREATIVE_RUN_LIFECYCLE_STATES =
  creativeLifecycleStateSchema.options;

const CREATIVE_RUN_LIFECYCLE_AUTHORITY = defineTransitionAuthority(
...
export function isCreativeRunLifecycleTransitionAllowed(
  from: string,
  to: string,
) {
...
const CREATIVE_RUN_WORKFLOW_AUTHORITY = defineTransitionAuthority(
...
export function isCreativeRunWorkflowTransitionAllowed(
  from: string,
  to: string,
) {
...
export const CREATIVE_RUN_VERIFICATION_STATES =
  adminVerificationStateSchema.options;

const CREATIVE_RUN_VERIFICATION_AUTHORITY = defineTransitionAuthority(
...
export function isCreativeRunVerificationTransitionAllowed(
  from: string,
  to: string,
) {
...
export const CREATIVE_PLACEMENT_VERIFICATION_STATES =
  adminVerificationStateSchema.options;

const CREATIVE_PLACEMENT_VERIFICATION_AUTHORITY = defineTransitionAuthority(
...
export function isCreativePlacementVerificationTransitionAllowed(
  from: string,
  to: string,
) {
...
const CREATIVE_RUN_ITEM_AUTHORITY = defineTransitionAuthority(
...
export const CONTROL_PLANE_COMMAND_STATES =
  adminControlPlaneCommandStatusSchema.options;

const CONTROL_PLANE_COMMAND_AUTHORITY = defineTransitionAuthority(
...
export function isControlPlaneCommandTransitionAllowed(
  from: string,
  to: string,
) {
...
export const CONTROL_PLANE_COMMAND_ATTEMPT_STATES =
  adminControlPlaneCommandAttemptStatusSchema.options;

const CONTROL_PLANE_COMMAND_ATTEMPT_AUTHORITY = defineTransitionAuthority(
...
export function isControlPlaneCommandAttemptTransitionAllowed(
  from: string,
  to: string,
) {
