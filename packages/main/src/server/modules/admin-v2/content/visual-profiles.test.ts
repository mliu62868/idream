...
import {
  GET as listVisualProfilesRoute,
  POST as createVisualProfileRoute,
} from "@/app/api/v2/admin/content/characters/[id]/visual-profiles/route";

/** SPEC: the two Visual Identity Route Handlers, keyed by the character id in the path. */
function listCharacterVisualProfiles(request: Request, characterId: string) {
  return listVisualProfilesRoute(request, { params: Promise.resolve({ id: characterId }) });
}

function createCharacterVisualProfile(request: Request, characterId: string) {
  return createVisualProfileRoute(request, { params: Promise.resolve({ id: characterId }) });
}
...
function makeRequest(
  ...
  return new Request(`http://localhost/api/v2/admin/content/characters${path}`, {
    method,
    headers,
...
/** Normalises the Route Handler envelope into the success/failure shape the assertions use. */
