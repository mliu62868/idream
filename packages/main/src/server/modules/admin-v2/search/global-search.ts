...
export async function globalAdminSearch(request: Request) {
  ...
  const [customers, characters, runs, cases, incidents, jobs] = await Promise.all([
    ...
    permissions.has("character.project.read") && (characterScope === null || characterScope.size > 0)
      ? prisma.character.findMany({ where: { deletedAt: null, ...(characterScope === null ? {} : { id: { in: [...characterScope] } }), OR: [{ id: contains }, { name: contains }, { description: contains }] }, orderBy: [{ updatedAt: "desc" }, { id: "desc" }], take: query.limit })
      : [],
    permissions.has("creative.run.read")
      ? prisma.contentProductionBatch.findMany({ where: { OR: [{ id: contains }, { title: contains }, { targetId: contains }] }, orderBy: [{ updatedAt: "desc" }, { id: "desc" }], take: query.limit })
      : [],
