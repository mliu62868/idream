...
import { materializeMetricSnapshots, readMetricDashboard, validateMetricDefinitions } from "@/server/modules/admin-v2/metrics/query";
import { summarizeMetricRefresh } from "@/server/modules/admin-v2/metrics/refresh";

async function main() {
  const { values } = parseArgs({
    args: process.argv.slice(2),
    options: { check: { type: "boolean" }, validate: { type: "boolean" }, "as-of": { type: "string" }, help: { type: "boolean" } },
    strict: true,
  });
  if (values.help) {
    process.stdout.write("Usage: metrics:refresh [--check | --validate] [--as-of ISO_TIMESTAMP]\n"
      + "Default: materialize current facts and quality evidence. --check: read only. --validate: explicitly validate definitions against current authoritative facts, then refresh.\n"
      + "Exit codes: 0 = official metrics decision-ready; 2 = report available but blocked; 1 = execution error.\n");
    return;
  }
  if (values.check && values.validate) throw new Error("--check and --validate cannot be combined");
  ...
  const validation = values.validate ? await validateMetricDefinitions(prisma, asOf) : undefined;
  ...
  process.stdout.write(`${JSON.stringify({
    mode: values.check ? "check" : values.validate ? "validate" : "refresh",
    validation,
