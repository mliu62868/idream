...
export const navItems: NavItem[] = [
  ...
  item({ id: "moderation", label: "Moderation Cases", href: "/admin/cases?view=moderation", icon: ShieldAlert, group: "Customer Operations", permissions: ["safety.review.read"] }),
  item({ id: "support", label: "Support Cases", href: "/admin/cases?view=support", icon: Ticket, group: "Customer Operations", permissions: ["support.request.read"] }),
  item({ id: "risk", label: "Risk Cases", href: "/admin/cases?view=risk", icon: AlertTriangle, group: "Customer Operations", permissions: ["user.read"] }),
  ...
  item({ id: "generation/jobs", label: "Jobs & Incident Signals", href: "/admin/ops/jobs", icon: Activity, group: "Platform Operations", permissions: ["generation.job.read"] }),
...
const SECTION_ALIASES: Record<string, string> = { "generation/models": "generation/config" };
...
function canonicalSection(path: string, query: URLSearchParams): AdminPath | null {
  if (path === "cases") {
    const sectionId = { support: "support", risk: "risk" }[query.get("view") ?? ""] ?? "moderation";
