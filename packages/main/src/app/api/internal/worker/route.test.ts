...
// SPEC: the internal worker endpoint is the Vercel-Cron entrypoint that drains
// a batch of BullMQ jobs. Exercises the shared http `handle()` wrapper (auth
// error → fail(); success → ok()) through the real route handler.
