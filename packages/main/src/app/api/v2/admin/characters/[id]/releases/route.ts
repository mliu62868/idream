...
import { createCharacterRelease } from "@/server/modules/admin-v2/characters/release-lifecycle";
import {
  actorWithPermission,
  jsonBody,
...
export async function POST(
  ...
  return adminV2Route(request, async () => {
    ...
    if (body.confirmation !== `${id}:publish`)
      throw Errors.badRequest(
        "Confirmation did not match Character publish target",
      );
    ...
    return executeAtomicIdempotentMutation({
      ...
      idempotencyKey,
      requestId,
      commandType: "character.release.prepare",
      target: { type: "character", id },
      expectedVersion: body.entityVersion,
      payload: body,
      mutate: async (tx) => {
        return createCharacterRelease(
          {
            request,
            actor,
            requestId,
