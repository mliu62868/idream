...
export async function drainCharacterReleaseCommands(
  db: PrismaClient,
  ...
  for (const command of commands) {
    const result = await executeCharacterReleaseCommand(db, {
...
export async function runAdminCommandWorkerLoop() {
  ...
  while (running) {
    let processed = 0;
    try {
      const result = await drainCharacterReleaseCommands(prisma, { workerId });
