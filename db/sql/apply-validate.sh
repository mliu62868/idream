...
must_reject "UPDATE billing view"          "UPDATE billing.chat_entitlement_view SET unlimited_messages=true;"
must_reject "SELECT retired memory multiplier" "SELECT memory_multiplier FROM billing.chat_entitlement_view LIMIT 1;"
...
must_reject "EXECUTE ledger redactor"      "SELECT chat.redact_file_mutation_payload('memory_extract','{\"kind\":\"memory_extract\"}'::jsonb);"
...
must_reject_projector "projector EXECUTE ledger redactor" "SELECT chat.redact_file_mutation_payload('memory_extract','{\"kind\":\"memory_extract\"}'::jsonb);"
