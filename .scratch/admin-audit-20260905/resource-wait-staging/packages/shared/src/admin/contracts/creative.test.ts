...
import {
  ...
  creativeRunItemExecutionStateSchema,
} from "./creative";

describe("Creative Run create contract", () => {
  it("preserves unknown execution as a distinct read projection", () => {
    expect(creativeRunItemExecutionStateSchema.parse("unknown")).toBe("unknown");
    expect(creativeRunItemExecutionStateSchema.parse("failed")).toBe("failed");
  });
