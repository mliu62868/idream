...
describe("generation terminal finalize payload", () => {
  it("preserves native provider request IDs and structured accounting through finalize", () => {
    const usage = { providerRequestIds: ["comfy-prompt-1"], timings: { executionMs: 331_959 }, priced: false, cost: null };
    expect(aiFinalizePayloadSchema.parse({ ...payloads[0], ...terminalEvidence, usage })).toMatchObject({ usage });
  });

  it.each([undefined, () => 1, Symbol("invalid"), Number.NaN])("rejects non-JSON provider usage values", (value) => {
    expect(aiFinalizePayloadSchema.safeParse({ ...payloads[0], ...terminalEvidence, usage: { nested: [value] } }).success).toBe(false);
  });
