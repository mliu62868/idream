...
const originalImageTimeout = process.env.GEN_IMAGE_TIMEOUT_MS;
...
afterEach(() => {
  if (originalImageTimeout === undefined) delete process.env.GEN_IMAGE_TIMEOUT_MS;
  else process.env.GEN_IMAGE_TIMEOUT_MS = originalImageTimeout;
...
describe("generation environment", () => {
  ...
  it.each(["invalid", "0", "-1", "1.5"])("rejects invalid image execution budget %s", (value) => {
    process.env.GEN_IMAGE_TIMEOUT_MS = value;
    expect(() => env.IMAGE_TIMEOUT_MS).toThrow("GEN_IMAGE_TIMEOUT_MS must be a positive integer");
  });
